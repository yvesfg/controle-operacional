      ...
      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:8,flexWrap:"wrap"}}>
        <div style={{minWidth:0,flex:1}}>
          ...
          <div style={{fontSize:11,color:"var(--text3)",fontFamily:"'DM Mono',monospace",letterSpacing:"0.04em",marginTop:2}}>
            DT {r.dt}
            {r.nf && <span style={{marginLeft:6,color:"var(--text3)"}}>· NF {r.nf}</span>}
...
export default function OcorrenciasView({ dados=[], diariasData, filtroOcorr, setFiltroOcorr, abrirDetalhe, t, isMobile, motoristas=[], onSalvarOcorrencia, css={} }) {
  ...
  return (
    <div style={{padding:isMobile?"12px":"16px 24px",width:"100%",boxSizing:"border-box"}}>
      ...
      {/* Busca + Filters row */}
      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6,padding:"7px 12px",background:"var(--card)",border:"1px solid var(--border)",borderRadius:10,flexWrap:"wrap"}}>
      ...
      {filtered.length===0?(
        ...
        <div style={{display:"grid",gridTemplateColumns:`repeat(${isMobile?Math.min(ocorrCols,2):ocorrCols},minmax(0,1fr))`,gap:isMobile?8:10}}>
