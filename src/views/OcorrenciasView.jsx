...
function OcorrCard({ entry, onOpen }) {
...
export default function OcorrenciasView({ dados = [], diariasData, filtroOcorr, setFiltroOcorr, abrirDetalhe, t, isMobile }) {
  ...
  // ── Render ─────────────────────────────────────────────────────────────────
  return (
    <div style={{ padding: isMobile ? "16px" : "24px", maxWidth: 900, margin: "0 auto" }}>
      ...
      {filtered.length === 0 ? (
        ...
        <div style={{
          ...
          {filtered.map((entry, i) => (
            <OcorrCard key={i} entry={entry} onOpen={abrirDetalhe} />
          ))}
        </div>
      )}
    </div>
