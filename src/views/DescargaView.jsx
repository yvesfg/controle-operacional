...
export default function DescargaView({ ctx }) {
  ...
  // Campo usado pro filtro de período/ano/mês por aba: Hoje/Atrasado/Aguardando pivotam em data_agenda;
  // Carrega Hoje/Sem Motorista pivotam em data_carr (evita comparar "amanhã" contra a data de carregamento).
  const campoOrdenacao = r => {
    const usaCarr = dscTab === "carrega" || dscTab === "semMotorista";
    return (usaCarr ? (r.data_carr || r.data_agenda) : (r.data_agenda || r.data_carr)) || "";
  };
  ...
  return (
          <div>
            ...
            {(()=>{
              ...
              const anosD=[...new Set(_tabAll.map(r=>{const ym=_pym(campoOrdenacao(r));return ym?.ano;}).filter(Boolean))].sort((a,b)=>b.localeCompare(a));
              const mesesD=[...new Set(_tabAll.filter(r=>{if(!dscFiltroAno)return true;const ym=_pym(campoOrdenacao(r));return ym?.ano===dscFiltroAno;}).map(r=>{const ym=_pym(campoOrdenacao(r));return ym?.mes;}).filter(Boolean))].sort();
              ...
              return (
                <FilterBar
                  ...
                  onQuickDate={dscTab!=="semMotorista"?setDscData:undefined}
                ...
                {!rodorricaFileName ? (
                  ...
                  <div style={{marginBottom:16}}>
                    ...
                    {rodorricaPeriodoModal&&(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.55)",zIndex:"var(--z-modal)",display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>setRodorricaPeriodoModal(false)}>
                      <div className="co-modal-box co-modal-box--lg co-modal-box--primary" style={{ maxWidth: 420, minWidth: 340 }} onClick={e=>e.stopPropagation()}>
