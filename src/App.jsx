          ...
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:isMobile?4:6,marginBottom:14}}>
              {[
                ...
                <div key={tb.k} onClick={()=>{setDFiltro(dFiltro===tb.k?"todos":tb.k);setDSubTab("resumo");}} style={{border:`1.5px solid ${dFiltro===tb.k?tb.cor:t.borda}`,borderRadius:8,padding:isMobile?"10px 5px":"18px 10px",cursor:"pointer",background:dFiltro===tb.k?tb.bg:t.card2,display:"flex",flexDirection:"column",alignItems:"center",gap:4,transition:"all .2s",minWidth:0}}>
      ...
      {wppTipoOpen && (
        ...
        >
          <div style={{background:t.card,border:`1px solid ${t.borda}`,borderRadius:20,width:"100%",maxWidth:460,boxShadow:`0 32px 64px rgba(0,0,0,.4)`,overflow:"hidden",animation:"slideUp .22s cubic-bezier(.34,1.1,.64,1)"}}>
            ...
            {/* DT Context */}
            {buscaResult ? (
              <div style={{padding:"10px 20px",background:"rgba(37,211,102,.07)",borderBottom:`1px solid ${t.borda}`,display:"flex",alignItems:"center",gap:8}}>
                {hIco(<><rect x="1" y="3" width="15" height="13" rx="2"/><path d="m16 8 4 2 3 3v4h-7"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/>
                </>,"#25D366",14)}
                <div>
                  <div style={{fontSize:12,fontWeight:700,color:"#25D366"}}>DT {buscaResult.dt} · {buscaResult.nome||"—"}</div>
                  {buscaResult.placa&&<div style={{fontSize:10,color:t.txt2,fontFamily:"'DM Mono',monospace"}}>{buscaResult.placa}{buscaResult.origem&&" · "+buscaResult.origem}</div>}
                </div>
              </div>
            ) : (
              <div style={{padding:"10px 20px",background:`rgba(240,185,11,.07)`,borderBottom:`1px solid ${t.borda}`,display:"flex",alignItems:"center",gap:8}}>
                {hIco(<><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/>
                </>,t.ouro,14)}
                <div style={{fontSize:12,color:t.ouro,fontWeight:600}}>Busque um registro na Planilha primeiro</div>
              </div>
            )}
            {/* Options */}
