...
export default function App() {
  ...
  const descargaData = useMemo(() => {
    ...
    return { hoje, atrasados, aguardando, carregaHoje };
  }, [DADOS, dscData]);

  // Conferência Planilha RODORRICA (Descarga + Stretch)
  const rodorricaResultado = useMemo(() => {
    ...
    rodorricaRows.forEach(row => {
      ...
      if (!dt) return;
      if (!planMap[dt]) planMap[dt] = { dt, descarga:0, stretch:0, linhas:[] };
      const tipo = String(row.tipo || '').toUpperCase();
      const val = Number(row.valorAprovado) || 0;
      if (tipo === 'DESCARGA') planMap[dt].descarga += val;
      else if (tipo === 'STRECH') planMap[dt].stretch += val;
      planMap[dt].linhas.push(row);
    });
    const apontMap = {};
    apontItems.forEach(a => {
      const dt = String(a.dt_rel || a.dt_relacionado || '').trim();
      if (!dt) return;
      if (!apontMap[dt]) apontMap[dt] = { descarga:0, stretch:0 };
      const val = Number(String(a.valor || '').replace(',', '.')) || 0;
      const tipo = (a.tipo || '').toLowerCase();
      if (tipo === 'descarga') apontMap[dt].descarga += val;
      else if (tipo === 'stretch') apontMap[dt].stretch += val;
    });
    const planDTs = new Set(Object.keys(planMap));
    const linhas = Object.values(planMap).map(p => {
      const ap = apontMap[p.dt];
      const totalPlan = p.descarga + p.stretch;
      const totalApont = ap ? ap.descarga + ap.stretch : 0;
      const diff = totalPlan - totalApont;
      const diffDesc = p.descarga - (ap ? ap.descarga : 0);
      const diffStr  = p.stretch  - (ap ? ap.stretch  : 0);
      const appReg = DADOS.find(r => String(r.dt||'').trim() === p.dt);
      if (!ap || totalApont === 0)
        return { ...p, conf:'SEM_APONT', totalPlan, totalApont:0, diff:totalPlan, diffDesc:p.descarga, diffStr:p.stretch, appReg };
      return Math.abs(diff) < 1
        ? { ...p, conf:'BATE',    totalPlan, totalApont, diff:0,  diffDesc, diffStr, appReg }
        : { ...p, conf:'DIVERGE', totalPlan, totalApont, diff,    diffDesc, diffStr, appReg };
    });
    const foraPlanilha = Object.entries(apontMap)
      .filter(([dt, ap]) => !planDTs.has(dt) && (ap.descarga > 0 || ap.stretch > 0))
      .map(([dt, ap]) => {
        const appReg = DADOS.find(r => String(r.dt||'').trim() === dt);
        const totalApont = ap.descarga + ap.stretch;
        return { dt, descarga:0, stretch:0, linhas:[],
          conf:'FORA_PLANILHA', totalPlan:0, totalApont, diff:-totalApont,
          diffDesc:-ap.descarga, diffStr:-ap.stretch, appReg };
      });
    ...
    return {
      ...
      totais: {
        bate: tot('BATE'), diverge: tot('DIVERGE'),
        semApont: tot('SEM_APONT'), foraPlanilha: tot('FORA_PLANILHA'),
        valorEmRisco: todos.filter(x=>x.conf==='DIVERGE').reduce((s,x)=>s+Math.abs(x.diff||0),0),
        totalPlanilha: todos.reduce((s,x)=>s+(x.totalPlan||0),0),
        totalApontamentos: todos.reduce((s,x)=>s+(x.totalApont||0),0),
      },
    };
  }, [rodorricaRows, apontItems, DADOS]);
    ...
    reader.onload = (ev) => {
      try {
        ...
        const rows = json.map(r => ({
          ...
          valorAprovado: Number(r['VALOR APROVADO'])||0,
          valorFinal:    Number(r['VALOR FINAL'])||0,
          centro:        String(r['CENTRO']||'').trim(),
          pagoOTM:       Number(r['Pago OTM'])||0,
        })).filter(r => r.dt && r.dt !== '' && r.dt !== 'undefined' && r.dt !== 'NaN' && /^\d+$/.test(r.dt));
          ...
          <div className="co-sidebar__logo">
            ...
            {(isWide?!sidebarCollapsed:mobileSidebarExpanded) && (
              <div style={{overflow:"hidden",flex:1,minWidth:0}}>
                ...
                <div className="co-sidebar__logo-sub">CONTROLE OPERACIONAL</div>
              </div>
            )}
            <button
        ...
        {isWide ? (
          ...
          <>
            <div>
              ...
              <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:"var(--text3)",letterSpacing:"0.04em",textTransform:"uppercase",marginTop:2}}>CONTROLE OPERACIONAL</div>
            </div>
      ...
      <div style={{padding:(activeTab==="planilha"||activeTab==="relatorios")?(isMobile?"0 0 68px":"0"):(isMobile?"16px 16px 68px":"16px 16px 24px"),maxWidth:(activeTab==="planilha"||activeTab==="relatorios"||activeTab==="dashboard"||activeTab==="diarias"||activeTab==="descarga")?"100%":1100,margin:"0 auto",animation:"fadeIn .2s",...(activeTab==="relatorios"?{display:"flex",flexDirection:"column",height:"calc(100vh - 56px)",overflow:"hidden"}:{})}}>
                    ...
                    {kpis.map((k,i)=>(
                      ...
                      >
                        <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:isMobile?3:8}}>
                          <div style={{fontFamily:"'DM Mono',monospace",fontSize:isMobile?9:11,textTransform:"uppercase",letterSpacing:"0.06em",color:"var(--text3)",fontWeight:400,lineHeight:1.4,paddingRight:4}}>{k.label}</div>
                        ...
                        <div style={{fontFamily:"'Space Grotesk',sans-serif",fontSize:isMobile?18:28,fontWeight:700,letterSpacing:"-0.04em",color:k.color,lineHeight:1,marginBottom:2}}>{k.value}</div>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"2fr 1fr 1fr",gap:12,marginBottom:12}}>
                    ...
                    <div style={{...css.card,padding:16}}>
                      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:10}}>
                        <div>
                          <div style={{fontFamily:"'DM Mono',monospace",fontSize:11,textTransform:"uppercase",letterSpacing:"0.06em",color:"var(--text3)",fontWeight:400}}>Evolução do Período</div>
                        ...
                        <div style={{textAlign:"right"}}>
                          <div style={{fontFamily:"'Space Grotesk',sans-serif",fontSize:28,fontWeight:700,letterSpacing:"-0.04em",color:"var(--text)",lineHeight:1}}>{heroNum}</div>
                        ...
                        return (
                          <div key={nome} style={{marginBottom:10}}>
                            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
                              ...
                              <span style={{fontFamily:"'DM Mono',monospace",fontSize:13,fontWeight:600,color:cores[i],flexShrink:0}}>{ct}</span>
                            </div>
                            <div style={{height:3,borderRadius:2,background:t.card2,overflow:"hidden"}}>
                      ...
                      <thead>
                        <tr>
                          {["DT","Motorista","Origem","Destino","Status",canFin?"CTE":""].filter(Boolean).map(h=>(
                            <th key={h} style={{padding:"0 6px 8px",textAlign:"left",fontFamily:"'DM Mono',monospace",fontSize:10,fontWeight:400,textTransform:"uppercase",letterSpacing:"0.06em",color:"var(--text3)",borderBottom:`1px solid ${t.borda}`,whiteSpace:"nowrap"}}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {recentesDash.length===0?(
                          ...
                          >
                            ...
                            <td style={{padding:"7px 6px"}}>
                              <span style={{padding:"2px 7px",borderRadius:4,fontFamily:"'DM Mono',monospace",fontSize:10,fontWeight:500,textTransform:"uppercase",color:sc(r.status),background:`${sc(r.status)}26`,whiteSpace:"nowrap",letterSpacing:"0.06em"}}>{(r.status||"–").slice(0,9)}</span>
                            </td>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginBottom:10}}>
                      {[{l:"No Prazo",v:diariasData.ok,c:t.verde,bg:`rgba(2,192,118,.08)`},{l:"Perdeu Agenda",v:diariasData.atraso,c:t.danger,bg:`rgba(246,70,93,.08)`},{l:"Aguardando",v:diariasData.pend,c:t.ouro,bg:`rgba(240,185,11,.08)`}].map(d=>(
                        <div key={d.l} onClick={()=>setActiveTab("diarias")} style={{background:d.bg,borderRadius:8,padding:"8px 6px",textAlign:"center",cursor:"pointer"}}>
                          <div style={{fontFamily:"'Space Grotesk',sans-serif",fontSize:20,fontWeight:700,letterSpacing:"-0.03em",color:d.c,lineHeight:1}}>{d.v}</div>
        ...
        {activeTab === "diarias" && (
          <div style={{minHeight:"calc(100vh - 140px)"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:isMobile?6:10,marginBottom:14}}>
              {/* No Prazo */}
              <div style={{...css.kpi(t.verde),cursor:"pointer",outline:dFiltro==="ok"?`2px solid ${t.verde}`:"none",padding:isMobile?"10px 5px":"18px 10px",display:"flex",flexDirection:"column",alignItems:"center",gap:4}} onClick={()=>{setDFiltro(dFiltro==="ok"?"todos":"ok");setDSubTab("resumo");}}>
                <div style={{width:isMobile?28:40,height:isMobile?28:40,borderRadius:"50%",background:"rgba(2,192,118,.12)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                ...
                <div style={{fontFamily:"'Space Grotesk',sans-serif",fontSize:isMobile?22:34,fontWeight:700,letterSpacing:"-0.04em",lineHeight:1,color:t.verde}}>{diariasData.ok}</div>
                {!isMobile&&<div style={{fontSize:8,color:t.verde,opacity:.6}}>{dFiltro==="ok"?"● filtrado":"toque p/ filtrar"}</div>}
              </div>
              {/* Perdeu Agenda */}
              <div style={{...css.kpi(t.danger),cursor:"pointer",outline:dFiltro==="atraso"?`2px solid ${t.danger}`:"none",padding:isMobile?"10px 5px":"18px 10px",display:"flex",flexDirection:"column",alignItems:"center",gap:4}} onClick={()=>{setDFiltro(dFiltro==="atraso"?"todos":"atraso");setDSubTab("resumo");}}>
                <div style={{width:isMobile?28:40,height:isMobile?28:40,borderRadius:"50%",background:"rgba(246,70,93,.1)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                  {hIco(<><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></>,t.danger,isMobile?14:20)}
                </div>
                ...
                <div style={{fontFamily:"'Space Grotesk',sans-serif",fontSize:isMobile?22:34,fontWeight:700,letterSpacing:"-0.04em",lineHeight:1,color:t.danger}}>{diariasData.atraso}</div>
                {!isMobile&&<div style={{fontSize:8,color:t.danger,opacity:.6}}>{dFiltro==="atraso"?"● filtrado":"toque p/ filtrar"}</div>}
              </div>
              {/* Sem Descarga */}
              <div style={{...css.kpi(t.ouro),cursor:"pointer",outline:dFiltro==="pendente"?`2px solid ${t.ouro}`:"none",padding:isMobile?"10px 5px":"18px 10px",display:"flex",flexDirection:"column",alignItems:"center",gap:4}} onClick={()=>{setDFiltro(dFiltro==="pendente"?"todos":"pendente");setDSubTab("resumo");}}>
                <div style={{width:isMobile?28:40,height:isMobile?28:40,borderRadius:"50%",background:"rgba(240,185,11,.1)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                  {hIco(<><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></>,t.ouro,isMobile?14:20)}
                </div>
                ...
                <div style={{fontFamily:"'Space Grotesk',sans-serif",fontSize:isMobile?22:34,fontWeight:700,letterSpacing:"-0.04em",lineHeight:1,color:t.ouro}}>{diariasData.pend}</div>
              ...
              <div>
                ...
                {(()=>{
                  ...
                  return (
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10,padding:"7px 10px",background:t.card,border:`1px solid ${t.borda}`,borderRadius:10,flexWrap:"wrap"}}>
                      ...
                      <input type="date" value={dPlanFiltroIni} onChange={e=>setDPlanFiltroIni(e.target.value)} style={{...css.inp,padding:"4px 8px",fontSize:11,height:28,width:128,flexShrink:0}}/>
                      <span style={{fontSize:10,color:t.txt2,flexShrink:0}}>até</span>
                      <input type="date" value={dPlanFiltroFim} onChange={e=>setDPlanFiltroFim(e.target.value)} style={{...css.inp,padding:"4px 8px",fontSize:11,height:28,width:128,flexShrink:0}}/>
                      {temFiltroR && (
                        <button onClick={()=>{setDPlanFiltroAno("");setDPlanFiltroMes("");setDPlanFiltroOrigem("todas");setDPlanFiltroIni("");setDPlanFiltroFim("");}} style={{fontSize:9,padding:"4px 8px",borderRadius:6,border:`1px solid ${t.borda}`,background:"transparent",color:t.txt2,cursor:"pointer",fontFamily:"inherit"}}>&#10005; Limpar</button>
                      )}
                ...
                {diariaView==="linhas" ? (
                  ...
                  (()=>{
                    ...
                    return diariaNavDT?[..._dAll].sort((a,b)=>a.r.dt===diariaNavDT?-1:b.r.dt===diariaNavDT?1:0):_dAll;
                  })().slice(0,80).map((item,idx) => {
                  ...
                  <div style={{display:"grid",gridTemplateColumns:`repeat(${isMobile?Math.min(diariaCols,2):diariaCols},minmax(0,1fr))`,gap:isMobile?8:10,width:"100%"}}>
                    {(()=>{
                      ...
                      const _fRB=dPlanFiltroFim?new Date(dPlanFiltroFim+"T23:59:59"):null;
                      return diariasData.items.filter(i=>{
                        if(dFiltro!=="todos"&&i.tipo!==dFiltro&&!(dFiltro==="pendente"&&i.tipo==="pendente"))return false;
                        const{r}=i;const ym=_pRB(r.data_carr||r.data_agenda||"");
                        if(dPlanFiltroAno&&ym?.ano!==dPlanFiltroAno)return false;
                        if(dPlanFiltroMes&&ym?.mes!==dPlanFiltroMes)return false;
                        if(dPlanFiltroOrigem!=="todas"&&(r.origem||"").trim()!==dPlanFiltroOrigem)return false;
                        if(_iRB||_fRB){const d=ym?.full||null;if(!d)return false;if(_iRB&&d<_iRB)return false;if(_fRB&&d>_fRB)return false;}
                        return true;
                      });
                    })().slice(0,80).map((item,idx) => {
             ...
             {dSubTab === "planilha" && (() => {
              ...
              return (
                <div>
                  ...
                  {/* Tabela */}
                  <div style={{overflowX:"auto",borderRadius:"0 0 11px 11px",border:`1px solid ${t.borda}`,borderTop:"none",maxHeight:"calc(100vh - 260px)",overflowY:"auto"}}>
        ...
        {activeTab === "descarga" && (
          <div style={{minHeight:"calc(100vh - 140px)"}}>
            ...
            <div style={{display:"flex",justifyContent:"flex-end",marginBottom:8}}>
              <ExportMenu
                dados={dscTab==="hoje"?descargaData.hoje:dscTab==="aguardando"?descargaData.aguardando:dscTab==="carrega"?descargaData.carregaHoje:descargaData.atrasados}
            ...
            <div style={{display:"grid",gridTemplateColumns:isMobile?"repeat(3,1fr)":"repeat(5,1fr)",gap:isMobile?4:6,marginBottom:12}}>
              {[
            ...
            {/* Filtros Descarga */}
            {(()=>{
              const _tabAll=dscTab==="hoje"?descargaData.hoje:dscTab==="aguardando"?descargaData.aguardando:dscTab==="carrega"?descargaData.carregaHoje:descargaData.atrasados;
              ...
              const _cnt=_tabAll.filter(r=>{const ym=_pymF(r.data_carr||r.data_agenda||"");if(dscFiltroAno&&ym?.ano!==dscFiltroAno)return false;if(dscFiltroMes&&ym?.mes!==dscFiltroMes)return false;if(dscFiltroOrigem!=="todas"&&(r.origem||"").trim()!==dscFiltroOrigem)return false;if(_iniC||_fimC){const d=ym?.full||null;if(!d)return false;if(_iniC&&d<_iniC)return false;if(_fimC&&d>_fimC)return false;}return true;}).length;
              return (
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10,padding:"7px 10px",background:t.card,
                  ...
                  <input type="date" value={dscFiltroIni} onChange={e=>setDscFiltroIni(e.target.value)}
                    style={{...css.inp,padding:"4px 8px",fontSize:11,height:28,width:128,flexShrink:0}}/>
                  <span style={{fontSize:10,color:t.txt2,flexShrink:0}}>até</span>
            ...
            <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:12,flexWrap:"wrap"}}>
              ...
              {descargaView==="blocos" && (
                <>
                  <span style={{fontSize:9,color:t.txt2,marginLeft:6}}>Colunas:</span>
                  {[1,2,3,4].map(n => (
                    <button key={n} onClick={()=>{setDescargaCols(n);saveJSON("co_descarga_cols",n);}} style={{width:28,height:28,fontSize:11,fontWeight:700,border:`1.5px solid ${descargaCols===n?"var(--accent)":"var(--border)"}`,borderRadius:7,cursor:"pointer",background:descargaCols===n?"var(--accent2)":"var(--card2)",color:descargaCols===n?"var(--accent)":"var(--text2)",fontFamily:"'Space Grotesk',sans-serif"}}>{n}</button>
                  ))}
                </>
              )}
            </div>
            ...
            {descargaView==="linhas" ? (
              // ── MODO LINHAS (original) ──
              <>
                {(()=>{const _dl=dscTab==="hoje"?descargaData.hoje:dscTab==="aguardando"?descargaData.aguardando:dscTab==="carrega"?descargaData.carregaHoje:descargaData.atrasados;const _iniL=dscFiltroIni?new Date(dscFiltroIni+"T00:00:00"):null;const _fimL=dscFiltroFim?new Date(dscFiltroFim+"T23:59:59"):null;const _pymL=s=>{if(!s)return null;if(/^\d{2}\/\d{2}\/\d{4}/.test(s)){const p=s.split("/");return{ano:p[2],mes:p[1],full:new Date(p[2]+"-"+p[1]+"-"+p[0]+"T00:00:00")};}if(/^\d{4}-\d{2}-\d{2}/.test(s)){const p=s.split("-");return{ano:p[0],mes:p[1],full:new Date(s+"T00:00:00")};}return null;};const _fl=_dl.filter(r=>{const ym=_pymL(r.data_carr||r.data_agenda||"");if(dscFiltroAno&&ym?.ano!==dscFiltroAno)return false;if(dscFiltroMes&&ym?.mes!==dscFiltroMes)return false;if(dscFiltroOrigem!=="todas"&&(r.origem||"").trim()!==dscFiltroOrigem)return false;if(_iniL||_fimL){const d=ym?.full||null;if(!d)return false;if(_iniL&&d<_iniL)return false;if(_fimL&&d>_fimL)return false;}return true;});return descargaNavDT?[..._fl].sort((a,b)=>a.dt===descargaNavDT?-1:b.dt===descargaNavDT?1:0):_fl;})().slice(0,50).map((r,i) => {
                  const da = parseData(r.data_agenda);
            ...
            {(()=>{const _de=dscTab==="hoje"?descargaData.hoje:dscTab==="aguardando"?descargaData.aguardando:dscTab==="carrega"?descargaData.carregaHoje:descargaData.atrasados;const _iniE=dscFiltroIni?new Date(dscFiltroIni+"T00:00:00"):null;const _fimE=dscFiltroFim?new Date(dscFiltroFim+"T23:59:59"):null;const _pymE=s=>{if(!s)return null;if(/^\d{2}\/\d{2}\/\d{4}/.test(s)){const p=s.split("/");return{ano:p[2],mes:p[1],full:new Date(p[2]+"-"+p[1]+"-"+p[0]+"T00:00:00")};}if(/^\d{4}-\d{2}-\d{2}/.test(s)){const p=s.split("-");return{ano:p[0],mes:p[1],full:new Date(s+"T00:00:00")};}return null;};return _de.filter(r=>{const ym=_pymE(r.data_carr||r.data_agenda||"");if(dscFiltroAno&&ym?.ano!==dscFiltroAno)return false;if(dscFiltroMes&&ym?.mes!==dscFiltroMes)return false;if(dscFiltroOrigem!=="todas"&&(r.origem||"").trim()!==dscFiltroOrigem)return false;if(_iniE||_fimE){const d=ym?.full||null;if(!d)return false;if(_iniE&&d<_iniE)return false;if(_fimE&&d>_fimE)return false;}return true;}).length;})() === 0 && (
              <div style={css.empty}><div style={{fontSize:36,marginBottom:10}}>{dscTab==="hoje"?"📅":dscTab==="aguardando"?"⏳":"✅"}</div><h3 style={{fontFamily:"'Space Grotesk',sans-serif",fontSize:15,fontWeight:600,letterSpacing:"-0.02em",color:"var(--text2)"}}>{dscTab==="hoje"?"NENHUMA DESCARGA HOJE":dscTab==="aguardando"?"NENHUM AGUARDANDO AGENDA":dscTab==="carrega"?"NENHUM CARREGAMENTO HOJE":"SEM ATRASOS"}</h3></div>
            )}
            </>)}
              ...
              <div>
                {!rodorricaFileName ? (
                  <div
                    ...
                    <div style={{fontSize:11,color:t.txt2,marginTop:4}}>Arraste o .xlsx ou clique para selecionar — aba BASE</div>
                  </div>
                ) : (
                  <div style={{marginBottom:16}}>
                    <div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 12px",background:t.card,border:`1px solid ${t.borda}`,borderRadius:10,marginBottom:12}}>
                      ...
                      <button onClick={()=>{setRodorricaRows([]);setRodorricaFileName(null);setRodorricaFiltro("todos");}} style={{background:"transparent",border:`1px solid ${t.borda}`,borderRadius:6,padding:"2px 8px",fontSize:10,color:t.txt2,cursor:"pointer",fontFamily:"inherit"}}>&#10005; Limpar</button>
                    </div>
                    {rodorricaResultado && (()=>{
                      const {totais,linhas} = rodorricaResultado;
                      const fmtR = v => "R$ " + Number(v||0).toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2});
                      const STATUS_COR = {BATE:"#02c076",DIVERGE:"#f6465d",SEM_APONT:t.txt2,FORA_PLANILHA:"#f0b90b"};
                      const STATUS_LABEL = {BATE:"✓ BATE",DIVERGE:"✗ DIVERGE",SEM_APONT:"— SEM APONT.",FORA_PLANILHA:"◊ FORA PLAN."};
                      const KPIS = [
                        {k:"BATE",l:"Bate",c:"#02c076",bg:"rgba(2,192,118,.08)",v:totais.bate},
                        {k:"DIVERGE",l:"Diverge",c:"#f6465d",bg:"rgba(246,70,93,.08)",v:totais.diverge},
                        {k:"SEM_APONT",l:"Sem Apontamento",c:t.txt2,bg:t.card2,v:totais.semApont},
                        {k:"FORA_PLANILHA",l:"Fora Planilha",c:"#f0b90b",bg:"rgba(240,185,11,.08)",v:totais.foraPlanilha},
                      ];
                      const filtrado = rodorricaFiltro === "todos" ? linhas : linhas.filter(x=>x.conf===rodorricaFiltro);
                      return (
                        <div>
                          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8,marginBottom:12}}>
                            {KPIS.map(k=>(
                              <div key={k.k} onClick={()=>setRodorricaFiltro(rodorricaFiltro===k.k?"todos":k.k)}
                                style={{background:k.bg,border:`1.5px solid ${rodorricaFiltro===k.k?k.c:t.borda}`,borderRadius:10,padding:"10px 14px",cursor:"pointer",textAlign:"center"}}>
                                <div style={{fontSize:22,fontFamily:"'Bebas Neue',sans-serif",color:k.c}}>{k.v}</div>
                                <div style={{fontSize:9,fontWeight:700,color:k.c,textTransform:"uppercase",letterSpacing:.6}}>{k.l}</div>
                              </div>
                            ))}
                          </div>
                          <div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap",alignItems:"center",padding:"8px 12px",background:t.card,border:`1px solid ${t.borda}`,borderRadius:10}}>
                            ...
                            <span style={{fontSize:10,color:t.txt2}}>App: <b style={{color:t.txt}}>{fmtR(totais.totalApontamentos)}</b></span>
                            {rodorricaFiltro!=="todos"&&<button onClick={()=>setRodorricaFiltro("todos")} style={{fontSize:9,padding:"3px 8px",borderRadius:6,border:`1px solid ${t.borda}`,background:"transparent",color:t.txt2,cursor:"pointer",fontFamily:"inherit"}}>&#10005; Limpar</button>}
                            <span style={{marginLeft:"auto",fontSize:10,color:t.txt2,fontWeight:600}}>{filtrado.length} DTs</span>
                          </div>
                          <div style={{overflowX:"auto"}}>
                            <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
                              <thead><tr style={{background:t.card2}}>
                                {["DT","Motorista","Desc.Plan","Desc.App","Str.Plan","Str.App","Total Plan","Total App","Dif.","Status"].map(h=>(
                                  <th key={h} style={{padding:"7px 10px",textAlign:"left",fontSize:9,fontWeight:700,color:t.txt2,textTransform:"uppercase",letterSpacing:.5,borderBottom:`1px solid ${t.borda}`,whiteSpace:"nowrap"}}>{h}</th>
                                ))}
                              </tr></thead>
                              <tbody>
                                {filtrado.map((row,i)=>{
                                  const cor = STATUS_COR[row.conf]||t.txt2;
                                  const nome = row.appReg?(row.appReg.nome||"—"):"—";
                                  const fV = v => v!==0?"R$ "+Number(v||0).toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2}):"—";
                                  const appDesc = row.totalApont>0 ? row.descarga-row.diffDesc : 0;
                                  const appStr  = row.totalApont>0 ? row.stretch -row.diffStr  : 0;
                                  return (
                                    <tr key={i} style={{borderBottom:`1px solid ${t.borda}`,background:i%2===0?t.bg:t.card}}>
                                      <td style={{padding:"6px 10px",fontWeight:700,color:t.azulLt}}>{row.dt}</td>
                                      <td style={{padding:"6px 10px",color:t.txt,maxWidth:120,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{nome}</td>
                                      <td style={{padding:"6px 10px",textAlign:"right",color:t.txt}}>{fV(row.descarga)}</td>
                                      <td style={{padding:"6px 10px",textAlign:"right",color:Math.abs(row.diffDesc)<1?t.txt:row.diffDesc>0?"#f6465d":"#f0b90b"}}>{fV(appDesc)}</td>
                                      <td style={{padding:"6px 10px",textAlign:"right",color:t.txt}}>{fV(row.stretch)}</td>
                                      <td style={{padding:"6px 10px",textAlign:"right",color:Math.abs(row.diffStr)<1?t.txt:row.diffStr>0?"#f6465d":"#f0b90b"}}>{fV(appStr)}</td>
                                      <td style={{padding:"6px 10px",textAlign:"right",fontWeight:700,color:t.txt}}>{fV(row.totalPlan)}</td>
                                      <td style={{padding:"6px 10px",textAlign:"right",fontWeight:700,color:t.txt}}>{fV(row.totalApont)}</td>
                                      <td style={{padding:"6px 10px",textAlign:"right",fontWeight:700,color:Math.abs(row.diff||0)<1?"#02c076":row.diff>0?"#f6465d":"#f0b90b"}}>{(row.diff||0)!==0?fV(row.diff):"—"}</td>
                                      <td style={{padding:"6px 10px"}}><span style={{fontSize:9,fontWeight:700,color:cor,background:`${cor}18`,border:`1px solid ${cor}44`,borderRadius:5,padding:"2px 7px",whiteSpace:"nowrap"}}>{STATUS_LABEL[row.conf]||row.conf}</span></td>
                                    </tr>
                                  );
                                })}
                              </tbody>
                            </table>
                            {!filtrado.length&&<div style={{textAlign:"center",padding:"32px 0",color:t.txt2,fontSize:12}}>Nenhum resultado</div>}
                          </div>
                        </div>
                      );
                    })()}
        ...
        {activeTab === "motoristas" && (()=>{
          ...
          return (
            <div>
              ...
              {motoristas.length === 0 ? (
                <div style={css.empty}><div style={{fontSize:36,marginBottom:10}}>🚛</div><h3 style={{fontFamily:"'Space Grotesk',sans-serif",fontSize:15,fontWeight:600,letterSpacing:"-0.02em",color:"var(--text2)"}}>SEM MOTORISTAS</h3><p style={{fontSize:11,color:t.txt2}}>Clique em + NOVO para cadastrar.</p></div>
