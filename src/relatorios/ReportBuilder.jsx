...
// Converte "dd/MM/yyyy" ou "yyyy-MM-dd" → "yyyy-MM-dd" para comparação
function toISO(d) {
  if (!d) return "";
...
export default function ReportBuilder({ dados = [], motoristas = [], apontItems = [], sgsItems = [], t, DESIGN, isMobile }) {
  ...
  const filteredData = useMemo(() => {
    ...
    if (filterModulo === "planilha" || filterModulo === "diarias") {
      ...
      if (filterDtDe)
        rows = rows.filter(r => toISO(r.data_agenda||r.data_carr||"") >= filterDtDe);
      if (filterDtAte)
        rows = rows.filter(r => toISO(r.data_agenda||r.data_carr||"") <= filterDtAte);
    }
