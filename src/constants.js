...
export const DEV_CHANGELOG = [
  {
    ...
    itens: [
      ...
      "FASE 1 APLICADA · Migration 070 rodada em producao (projeto qdrhkkjawklqfsoyxhpd) depois de conferir que as 4 RPCs em prod estavam identicas ao repo (sem drift). O SET dinamico com coluna date foi testado em prod num motorista descartavel: validade/nascimento gravam, campo vazio vira NULL, linha de teste removida. yf-ai-gateway commitado e pushado (8a454e9) — perfil cnh com data_nascimento e uf_emissao.",
      "FASE 2 (ficha do motorista) · A aba Cadastros > Motoristas virou a ficha completa, sem tela nova: botão Ler CNH no topo, seção CNH e dados pessoais (número, categoria, validade, UF, nascimento, gênero, função, qualificação) e seção Conjunto com uma linha por peça (cavalo + 3 carretas), cada uma com Ler CRLV próprio e os campos de marca/modelo/cor/ano/RENAVAM (tanque só no cavalo). Trocar a placa troca a peça: se a placa já existe no banco, os dados dela vêm junto e a linha diz 'já cadastrada'.",
      "FASE 2 · Selo de pendência no cabeçalho da ficha E na linha da lista ('falta 4' / 'completo'), calculado por pendenciasCadastro() em src/cadastroEmbarcadora.js — a MESMA função que a exportação vai usar, pra tela e arquivo nunca discordarem. Botão Concluir cadastro fica travado enquanto houver pendência (o título lista o que falta) e carimba cadastro_concluido_em. CPF repetido em cadastro novo avisa 'já cadastrado como X' e oferece abrir o existente, em vez de duplicar.",
      "FASE 2 · A DT entrou na tela: cada linha mostra a última DT do motorista (com +N quando há mais) e a busca aceita DT junto de nome/CPF/placa. O vínculo DT->motorista é o mesmo do ModalWhatsApp (CPF só-dígitos, depois nome exato, depois placa), num passo só sobre DADOS.",
      "VALIDADO · 19 checagens em node sobre cadastroEmbarcadora.js com os valores REAIS das duas planilhas: MAISCULINO/MAICULINO -> MASCULINO, MOTRISTA -> MOTORISTA, MARANHAO e 'IMPERATRIZ, MA' -> MA (cidade sozinha vira vazio), RENAVAM 549838660 -> 00549838660, CPF 844.951.701.04 -> 844.951.701-04. A tela foi renderizada num harness isolado (sem login, conn=null, removido depois): a ficha monta, o selo mostra 'Falta 10', digitar a placa revela o bloco do veículo com Ler CRLV, e o RENAVAM curto normaliza pra 11 dígitos ao sair do campo.",
      "FASE 3 (migration 071 + APLICADA em prod) · Tabela cadastro_templates: o LAYOUT virou dado, não código — embarcadora nova entra com um INSERT, sem deploy. Os dois modelos da Suzano já entraram semeados (3 seções cada, 29 e 28 colunas). Não é só a forma que muda entre eles: ESTADO CNH sai 'MA' num e 'MARANHÃO' no outro, TIPO DO VEÍCULO 'CAVALO' x 'TRACAO CAMINHAO TRATOR', tanque '540' x '560 LITROS' x 'X' — tudo isso é formato de coluna no template. Sem PII: RLS anon-permissivo igual ao de embarcadoras (006), não o modelo de RPC token-validada.",
      "FASE 3 (front) · Botão 'Cadastro embarcadora' na faixa de Cadastros > Motoristas abre um painel no mesmo padrão do 'Importar agenda': escolhe o modelo, lista as DTs da base com motorista, placas e selo, marca as que vão e gera o .xlsx. Quem tem pendência APARECE mas não pode ser marcado (o título diz o que falta) — arquivo incompleto volta da embarcadora e o trabalho é refeito. src/cadastroExport.js monta as matrizes e exportacao.js ganhou baixarXLSXAbas() pro modelo de 3 abas.",
      "REGRA · O conjunto do arquivo vem da DT (reg.placa/placa2/placa3), não do cadastro: o motorista troca de carreta entre uma carga e outra e a embarcadora quer o que rodou naquela viagem. O cadastro entra como fonte dos dados do documento. Placa que rodou e não está no cadastro de veículos vira casca, pra poder ACUSAR o que falta nela em vez de sumir do arquivo.",
      "VALIDADO contra os DOIS arquivos reais · Gerado em node a partir dos dados das planilhas e comparado linha a linha. Modelo de 3 abas: MOTORISTA, VEICULOS e CARRETA batem coluna a coluna (a única diferença é data, que sai como texto dd/MM/aaaa em vez de célula de data, e uma coluna fantasma vazia no original). Modelo de blocos: cabeçalhos e ordem dos blocos idênticos; as diferenças são as CORREÇÕES — MAISCULINO -> MASCULINO, MOTRISTA -> MOTORISTA, CPF 436.364.893.72 -> 436.364.893-72, telefone 62-99949-8023 -> (62) 99949-8023, RENAVAM 549838660 -> 00549838660. Painel renderizado em harness isolado (removido): fallback de template funciona sem banco, DT sem cadastro aparece como 'Motorista não cadastrado' e fica travada.",
      "FASE 4 (migration 072 + APLICADA em prod) · Tabela cadastro_envios guarda o ÚLTIMO envio por (embarcadora, dt) — upsert, não log: a pergunta da tela é 'esta DT já foi, e mudou desde então?', não 'quantas vezes foi'. O campo assinatura é o hash (djb2) dos campos que foram pro arquivo; comparar data de alteração não serviria, porque salvar o cadastro sem mexer em nada marcaria tudo como pendente de reenvio. Round-trip do upsert testado em prod (2 inserts da mesma DT = 1 linha, assinatura e placas atualizadas; linha de teste ZZ-TESTE removida).",
      "FASE 4 (front) · O painel de envio passou a mostrar por DT: 'novo', 'mudou desde 18/08/2026' ou 'enviado 20/08/2026' (com data e autor no título), mais a contagem '1 nova · 1 mudou · 1 já enviada'. Filtro 'Esconder o que já foi e não mudou' vem LIGADO — reenviar igual só faz a embarcadora receber repetido —, mas destrava. O registro do envio acontece DEPOIS de baixar o arquivo e não derruba a geração se falhar: o que o analista precisa é do .xlsx.",
      "FASE 4 · CNH VENCIDA agora é PENDÊNCIA, não aviso: a embarcadora recusa e o retrabalho é garantido. CNH vencendo em até 30 dias passa, mas avisa na ficha e na linha do envio ('CNH vence em 16d'), senão o cadastro é aceito e vira problema duas semanas depois.",
      "VALIDADO · 10 checagens em node: assinatura estável pra mesma carga, muda ao trocar a carreta NA DT e ao renovar a CNH; situação novo/igual/mudou; CNH vencida entra na lista de pendências e CNH válida não; dias pra vencer com sinal certo. Painel conferido em harness isolado (removido) com histórico simulado: a DT já enviada some por padrão e reaparece como 'enviado 20/08/2026' ao destravar o filtro; a de assinatura antiga aparece como 'mudou desde 18/08/2026'.",
      "ERRO DE MODELAGEM (Yves, olhando a tela: 'um mesmo motorista com o mesmo conjunto pode fazer varias viagens') · O envio estava chaveado pela DT, mas a embarcadora nao cadastra viagem — cadastra MOTORISTA + CONJUNTO. Duas consequencias: (1) o mesmo motorista com o mesmo cavalo e a mesma carreta aparecia uma vez por DT, e marcar tudo colocava o MESMO cadastro repetido N vezes dentro do arquivo; (2) o filtro 'ja foi e nao mudou' so escondia a MESMA DT, entao viagem nova do mesmo conjunto voltava como 'novo' e seria reenviada a toa. Quando ele troca uma peca, ai sim e cadastro novo — e isso a chave por DT tambem nao sabia dizer.",
      "FIX (migration 073 + APLICADA em prod) · A chave de cadastro_envios passou de (embarcadora, dt) para (embarcadora, assinatura); a coluna dt virou dts (as viagens em que aquele cadastro apareceu, so pro analista reconhecer a linha). Tabela estava vazia em producao, entao a troca foi direta, sem backfill. Indice novo por (embarcadora, motorista_id) pra responder a segunda pergunta da tela: 'este motorista ja foi alguma vez, com outro conjunto?'.",
      "FIX (front) · itensDoEnvio() agora AGRUPA por assinatura: cada linha e um cadastro e carrega as DTs ('DT 1353499 +2', lista inteira no titulo); a busca por DT casa com qualquer uma do grupo e a selecao passou a ser por cadastro. A situacao virou: 'ja enviado' quando a assinatura ja foi mandada, 'conjunto mudou desde dd/mm' quando o motorista ja foi mas com outro conjunto (o titulo mostra o conjunto anterior) e 'cadastro novo' quando nunca foi. Motorista sem cadastro agrupa por placa, que e o unico dado confiavel que resta.",
      "VALIDADO · 9 checagens em node com o cenario do Yves — 4 viagens do mesmo motorista, 3 com o conjunto A e 1 trocando a carreta: viram 2 cadastros; as 3 DTs entram numa linha so; o arquivo sai com 1 linha de motorista (antes sairiam 3); com o conjunto A ja enviado, viagem nova do mesmo conjunto fica 'igual' e a da carreta trocada fica 'mudou'; outro motorista com o mesmo conjunto continua 'novo'. Conferido tambem na tela em harness isolado (removido): com o filtro ligado sobra so a linha da carreta trocada.",
      "PEDIDO (Yves: 'faça isso para evitar erros') · O CPF/CNPJ do responsável passava qualquer coisa — na planilha real a carreta KDI4I79 traz '894864203', que não é CPF nem CNPJ. Agora o campo é OBRIGATÓRIO nos dois modelos (a coluna existe nos dois) e validado por DÍGITO VERIFICADOR, não por contagem: contar dígito deixaria passar número trocado, que é o erro comum de digitação. cpfValido/cnpjValido/cpfCnpjValido em cadastroEmbarcadora.js; o CPF do motorista passou a ser conferido pelo mesmo critério (era só comprimento).",
      "UX · Os dois campos ficam com borda de aviso e a frase do motivo enquanto o número não fecha ('CPF incompleto' x 'Dígito verificador não confere' x 'Não é um CPF nem um CNPJ válido'), e formatam sozinhos ao sair do campo quando validam. Vale na ficha do motorista (por peça do conjunto) e na aba Veículos. formatarCpfCnpj() virou fonte única — cadastroExport.js tinha uma cópia da formatação.",
      "IMPACTO MEDIDO em prod antes de subir · 849 motoristas, 7 sem CPF e 141 com CPF fora de 11 dígitos; 728 veículos, TODOS sem responsável. Não muda o quadro prático: os mesmos 728 já estavam pendentes por não terem marca nem RENAVAM (colunas novas da 070), então ninguém que hoje enviaria ficou travado por causa desta regra.",
      "VALIDADO · 14 checagens em node com os números REAIS das planilhas: os CPFs de PEDRO/RUGLES/HENGUEL e os CNPJs 02.191.966/0001-89, 00.249.353/0001-20 e 35.729.843/0002-99 passam; '894864203' reprova; sequência repetida reprova; um dígito trocado no CPF e no CNPJ reprova. '436.364.893.72' (o do ponto no lugar do hífen) É um CPF válido — só a pontuação estava errada, e o app reformata pra 436.364.893-72.",
      "BUG DE UX (Yves: 'achei o do motorista, mas nao dos veiculos') · O botao Ler CRLV existia na ficha do motorista, mas so aparecia DEPOIS de digitar a placa no slot — e e o CRLV que traz a placa, entao a ordem estava invertida e o botao ficava invisivel. Agora aparece sempre; ler o documento preenche a placa do slot junto com marca/modelo/cor/ano/RENAVAM.",
      "GAP · Cadastros > VEICULOS nao tinha intake NEM os campos da migration 070: aquela tela so editava tipo/eixos/carroceria/capacidade/vinculo, entao marca, modelo, cor, ano, RENAVAM, chassi, especie, tanque e CPF/CNPJ do responsavel so davam pra preencher pela ficha do motorista. Ganhou o botao Ler CRLV e a secao Documento do veiculo, e o salvar passou a gravar esses campos (vazio vira null — o RENAVAM tem indice unico onde string vazia repetida colidiria).",
      "DETALHE · Na aba Veiculos, a especie lida do CRLV classifica a peca ('CAMINHAO TRATOR' = cavalo, 'SEMI-REBOQUE' = carreta) SO em cadastro novo — reclassificar veiculo ja vinculado bagunçaria o conjunto do motorista. A linha da lista passou a mostrar marca/modelo e a marcar 'sem RENAVAM'. VALIDADO em harness (removido): nas DUAS telas o CRLV simulado preencheu tudo e o RENAVAM 429677057 entrou como 00429677057.",
      "LIMPEZA · viagensDoMotorista() saiu de cadastroEmbarcadora.js — nasceu na fase 2 e a versão final da tela não usa (a lista monta o índice DT->motorista num passo só)."
    ],
  },
  {
    ...
    itens: [
      ...
      "DIAGNOSTICO · Nao era volume de dados: as tabelas somam menos de 5 MB. Era escrita a toa do sync. O SyncSupabase*.gs manda TODAS as linhas de TODAS as abas a cada 15 min com resolution=merge-duplicates, que o PostgREST vira ON CONFLICT DO UPDATE incondicional -- linha identica e reescrita do mesmo jeito. Em 152 dias: 11.978.597 updates em 1.234 linhas (controle_operacional), 2.355.111 em 486 (avb) e 1.616.415 em 515 (maracanau) = ~16 MILHOES de UPDATE em ~2.200 linhas reais. Bate exato com linhas x 96 rodadas/dia x 152 dias. Agravante: em controle_operacional so 0,03% eram HOT (o trigger de updated_at mexe em coluna indexada), entao cada update reescrevia tambem as 10 entradas de indice. 23.049 autovacuums na tabela.",
      "SOBRE O UPGRADE · Org esta no plano free (Nano), onde nao existe compute add-on -- teria que ir pro Pro (US$25/mes) antes. E nao resolve a causa: o consumo cresce linear com o numero de linhas, entao o teto novo vira teto velho. Com a guarda, o Nano free sobra.",
      "FIX (migration 063) · RPC upsert_co_lote(p_tabela, p_rows): mesmo upsert, mas so grava quando a linha mudou. Semantica IDENTICA a do PostgREST de proposito -- conflito por dt (Imperatriz/Maracanau) ou codigo (AVB), coluna ausente no payload nao e tocada, coluna presente sobrescreve inclusive com string vazia. Agrupa o lote por assinatura de colunas (cada aba manda um conjunto diferente e coluna ausente nao pode virar NULL) e monta o SET por grupo. A guarda projeta a linha atual sobre exatamente as chaves que vieram e compara com o payload. O INSERT tem `not exists` antes do ON CONFLICT DO NOTHING de proposito: sem ele a insercao ESPECULATIVA das linhas ja existentes continuaria gravando heap+indices toda rodada.",
      "VALIDADO contra producao antes de trocar o .gs: 200 linhas reais de cada uma das 3 tabelas reenviadas identicas -> 0 updates nas tres. Numa linha de teste propria (dt ZZ-TESTE-063, removida depois): reenvio identico = 0 update; mudar `cliente` = 1 update com o valor novo; `nome` omitido do payload = preservado; `mdf` gravado direto no banco (simulando o app) e nunca presente no payload = preservado.",
      "FIX (migration 063) · Dropada a trigger trg_co_updated_at -- era duplicata exata de trg_updated_at (as duas so fazem NEW.updated_at = now()). Dropados 13 indices com 0 ou 1 scan desde 15/03 (6 em controle_operacional incl. idx_co_dt redundante com o UNIQUE em dt, 7 em maracanau): todo update nao-HOT reescreve TODOS os indices, entao so custavam escrita.",
      "FIX (migration 063) · Os 2 gatilhos comparavam com funcao aplicada na coluna, o que invalida qualquer indice comum: promover_dados_motorista_avb dava 573.518 seq scans em motoristas (486M tuplas lidas) e conciliar_sem_dt_trg 918.066 em controle_operacional_sem_dt. Criados idx_motoristas_cpf_digitos (expressao do CPF so-digitos) e idx_sem_dt_conciliacao (placa+origem normalizadas, parcial por status).",
      "MUDANCA DE COMPORTAMENTO (uma, e pra melhor) · updated_at para de avancar em sync sem alteracao. Hoje avancava a cada 15 min e significava 'o sync rodou'; passa a significar 'o dado mudou'. Quem quer saber quando o sync rodou ja tem co_config.gsheet_sync_status_*, gravado pelo proprio .gs. O caminho de escrita do app NAO muda: escreverFaturamentoNaPlanilha continua gravando na planilha antes do Supabase, e as telas continuam gravando por UPDATE/PATCH normal.",
      "FALTA YVES COLAR · Os 3 .gs atualizados no Apps Script (SyncSupabase.gs, SyncSupabase_AVB.gs, SyncSupabase_Maracanau.gs). Enquanto nao colar, o sync segue no POST direto e o IO continua queimando -- a RPC ja esta no banco esperando. O painel de status ganhou inseridos/atualizados/sem_mudanca: em regime normal quase tudo cai em sem_mudanca.",
      "BASELINE PRA MEDIR (pg_stat_reset e bloqueado no Supabase, entao anotado em vez de zerado) -- 14/08/2026 12:18 UTC: controle_operacional n_tup_upd=11.983.516 / autovacuum=23.049; avb=2.356.567; maracanau=1.617.917; motoristas seq_scan=573.518; sem_dt seq_scan=918.066. Comparar 24h depois de colar o .gs.",
      "MEDIDO depois que os 3 .gs entraram · Imperatriz 985 linhas enviadas -> 0 UPDATE; Maracanau 514 -> 0; AVB 487 -> 24 (mudanca real ou convergencia de primeira passada). Um ciclo completo das 3 bases custou +2 updates no total, contra ~2.235 antes. WAL por lote de 50 linhas caiu de 45.237 para 4.231 bytes (-89%) e paginas sujas por lote de 3,05 para 1,10. Nao precisou de upgrade de plano.",
      "UX (pedido Yves) · PREENCHER PELO BLOCO reorganizado. Antes era uma coluna so: o textarea e a conferencia dividiam a MESMA rolagem, entao conferir campo a campo empurrava o texto colado pra fora da tela. Agora sao dois paineis com rolagem INDEPENDENTE -- no desktop (>=1024) lado a lado, empilhados abaixo disso. O modal alarga de 640 pra 1040 so no desktop. O textarea tambem rola sozinho (altura clamp(220px,34vh,420px) no desktop; 190 no tablet; 150 no mobile) e continua com resize:vertical.",
      "REGRA DE LAYOUT · maxHeight em % nao funciona pra limitar o painel de cima no empilhado: a altura do corpo e indefinida no flex, entao a porcentagem e ignorada e o painel fica com a altura natural (medido: ficava com 55% e espremia a conferencia em 281px). O que resolve e prioridade de shrink -- painel de cima flex:0 1 auto/minHeight 96, conferencia flex:1 1 auto/minHeight 140. Passou a dar 62% pra conferencia.",
      "UX · CANCELAR virou LIMPAR: esvazia o bloco e devolve o foco pro campo SEM fechar, pra quem lanca varias DTs seguidas colar a proxima ali mesmo. Fechar e o X do cabecalho (que ja existia). Gravar com sucesso tambem nao fecha mais -- mostra o toast e limpa. Limpar zera modoManual de proposito: colar um bloco de Faturamento depois de um de Contratacao troca o tipo sozinho (validado).",
      "UX · Conferencia em tela estreita (<=600) virou 2 linhas por campo (rotulo + estado em cima, atual -> novo embaixo). Nas 4 colunas fixas de antes os valores viravam reticencias e a linha nao dizia nada. Painel da direita ganhou estado vazio ('Cole o bloco ao lado/acima') -- no desktop uma coluna vazia parecia tela quebrada.",
      "BREAKPOINT NOVO · isDesktop (>=1024) em useUIState, ao lado de isMobile (<=600) e isWide (>=768), propagado pelo ctx. 768 nao serve pra 2 colunas em modal: daria ~370px por coluna, estreito demais pra tabela de conferencia.",
      "FIX (Yves: 'clico no badge TRECHO NOVO e nao acontece nada, deveria me deixar decidir') · O badge nunca foi clicavel -- e um <span> informativo. A decisao existia SO no bloco do topo da tela, agrupado por trecho, entao quem via o aviso na linha la embaixo tinha que rolar ate o topo pra achar o botao, e o aviso parecia morto. Os outros dois problemas da mesma lista (cte_sem_contrato e cte_contrato_vinculado) ja traziam a acao inline na propria linha; trecho_nao_decidido era o unico que nao. Agora traz o mesmo padrao: 'E nossa operacao' / 'Nao e nossa' embaixo do badge.",
      "REGRA (nao mudou, mas agora esta escrito na tela) · A decisao e por (empresa, trecho), nao por contrato -- responder numa linha arrasta o trecho inteiro. A linha diz quantos contratos e quanto valor vao junto (ex.: 'vale para o trecho MABSOB inteiro (2 contratos, R$ 22.216,50)'), pra ninguem decidir 2 contratos achando que decidiu 1. O bloco do topo continua existindo pra decisao em lote: 26 contratos de MABCUC seguem sendo UMA pergunta, nao 26.",
      "DETALHE · 'E nossa operacao' nao faz a linha sumir: ela troca pro badge 'Contrato sem o CTe importado', que e o estado correto (e nossa, mas o CTe daquele mes ainda nao foi importado). So 'Nao e nossa' tira da fila.",
      "VALIDADO em node com o caso da tela (MABSOB, 2 contratos, nenhum CTe casando): a chave que a linha monta (MAB||MABSOB) casa com a do grupo mesmo com o trecho vindo em caixa mista do parser -- a regra e gravada em CAIXA ALTA por chaveRegra(), entao a linha normaliza com up() antes de comparar e antes de salvar. Sem isso o botao gravaria a regra e a linha nao sairia da fila. Render nao exercitado (a tela depende de conexao Supabase).",
      "FIX (Yves: 'icone no topo da sidebar recolhida esta cortado do lado esquerdo') · O div que embrulha 'YFGroup / CTRL OPERACIONAL' tem flex:1 e continuava no layout mesmo recolhido -- so o TEXTO dentro dele sumia (.co-sidebar--collapsed esconde -name e -sub, nunca o wrapper). Vazio, ele ainda engolia toda a folga da faixa, entao o justify-content:center nao tinha o que centralizar e o icone ia parar em left:0, colado na borda. Medido: icone a 0px da borda e 9,6px fora do eixo dos icones do nav. Novo .co-sidebar__logo-txt escondido quando recolhida: icone volta pra 9,6px de folga e centro EXATAMENTE no mesmo eixo do nav (diferenca 0,00px), nos dois temas.",
      "ACHADO DE PASSAGEM (nao mexi) · [data-theme=dark] .co-sidebar .co-sidebar__logo zera o padding horizontal (14px 0) e vence a regra de --collapsed (8px) por especificidade. Com o wrapper escondido o icone centra certo mesmo assim -- claro (padding 8px) e escuro (padding 0) dao os mesmos 9,6px --, entao a regra ficou como estava. Anotado caso apareca outro desalinhamento nessa faixa.",
      "CUIDADO QUE QUASE VIROU BUG · No desktop a sidebar expande no :hover e revela o nome com display:block !important. Esconder o WRAPPER mataria isso -- filho com !important nao aparece se o pai esta display:none. Por isso .co-sidebar__logo-txt entrou tambem nas duas regras de hover (generica e do tema escuro) que ja revelavam -name/-sub. Validado: recolhida o wrapper fica none; sob os seletores de hover volta pra block.",
      "NAO VALIDADO · o :hover com ponteiro de verdade (screenshot e coordenada indisponiveis nesta sessao) -- a checagem foi no nivel das regras CSS, medindo o wrapper com os seletores de hover aplicados.",
      "RESPOSTA (Yves: 'verifique a possibilidade de entrar com duas placas separadas por /') · JA FUNCIONAVA e continua: 'PLACAS: A / B' e 'PLACA: A / B' quebram em placa/placa2, aceitam ate 3 placas e tanto barra quanto virgula. Nao mexi nisso.",
      "FIX · O que NAO funcionava era o rotulo no PLURAL: 'PLACAS 01:'/'PLACAS 02:' caiam em 'Linha ignorada' (o parser so conhecia 'PLACA 01'), que e exatamente o aviso que aparecia na tela do Yves -- e a equipe digita no plural. Adicionados os rotulos plurais em placa/placa2/placa3, mais 'CAVALO' (CARRETA ja existia em placa2, era inconsistente). O guarda de PLACA 04 virou /^placas?/ pra pegar as duas formas.",
      "FIX (risco de perda de dado) · Data sem ano virava conflito falso E empobrecia o registro. Achado na propria tela do Yves: gravado '15/08/2026', bloco com '15/08', conferencia marcava DIFERENTE -- e clicar SOBRESCREVER gravaria '15/08' por cima, na planilha tambem. Agora o ano e completado em compararComRegistro (e nao no parser, porque so ali se conhece o registro): usa o ano que a DT JA tem naquele campo; campo vazio usa o ano corrente e a linha ganha o selo 'ano assumido'. Nunca grava data mais pobre que a existente. Cobre tambem '15/8' sem zero.",
      "UX · Botao dizia 'NADA A GRAVAR' mesmo havendo campos DIFERENTE esperando decisao -- parecia que nao havia o que fazer. Agora diz 'N CAMPO(S) DIFERENTE(S) -- MARQUE SOBRESCREVER' e volta pra 'GRAVAR N CAMPOS' quando o SOBRESCREVER e ligado.",
      "VALIDADO · Parser testado em node com 7 formatos de placa (plural, singular, uma linha com barra, virgula, cavalo/carreta, terceira placa, PLACA 04) e 6 de data. Tela validada em harness isolado com o bloco REAL do Yves (PLACAS no plural + datas sem ano): as 2 linhas que apareciam DIFERENTE viraram JA IGUAL, campo de data vazio preencheu 17/08/2026 com o selo 'ano assumido', e o rotulo do botao alterna certo ao marcar SOBRESCREVER. Harness removido, nao commitado.",
      "VALIDADO em navegador num harness isolado do modal (o login exige senha, entao a tela real nao da pra abrir aqui) -- harness criado e REMOVIDO depois, nao foi commitado. Medido em 1440/1024/1023/768/375: em 1024 vira grid (colunas 472+519, cabe na janela), em 1023 empilha; no mobile 375x812 os dois paineis rolam sozinhos, o textarea rola sozinho, o rodape fica dentro da tela e nem a pagina nem o overlay rolam. LIMPAR testado: zera o texto, foca o campo, desabilita sozinho e o painel da direita volta pro estado vazio.",
    ],
  },
  {
    ...
    itens: [
      ...
      "AUDITORIA (Yves: convite criado pro wandesonalvoradaf@gmail.com e a pessoa nao recebeu nada) · NAO era falha de entrega: o app NUNCA enviou e-mail de convite. hub_admin_convidar (migration 047) so faz INSERT em hub_convites -- sem pg_net, sem net.http, sem SMTP, sem trigger -- e 'Copiar aviso' so escreve no clipboard. Varredura em src/ e api/ por inviteUserByEmail, signInWithOtp, resend, sendgrid, nodemailer e smtp: zero ocorrencia. O unico 'enviar e-mail' do projeto e o legado do AdminView antigo, que abre o Gmail preenchido pra pessoa clicar em Enviar.",
      "FIX (decisao Yves: manter manual e deixar obvio) · O problema real era ninguem SABER que precisa avisar. Sem servico de e-mail novo: (1) depois de criar o convite aparece a caixa 'falta avisar a pessoa', com o texto pronto e os botoes Copiar aviso / WhatsApp (wa.me sem numero, escolhe o contato) / Gmail (compose preenchido -- quem clica em Enviar e o admin); (2) os mesmos 3 botoes no card de cada convite pendente, no lugar do 'Copiar aviso' sozinho; (3) linha embaixo do titulo 'Convites' dizendo que o app nao envia e-mail. A caixa nao aparece quando o e-mail JA tinha conta (nesse caso o acesso foi aplicado na hora e nao depende de aviso).",
      "REGRA (que ja valia e nao estava dita na tela) · O convite so casa se a pessoa entrar com o Google DAQUELE e-mail; com outra conta ela cai na fila de aprovacao. O aviso e a caixa nova falam isso explicitamente.",
      "FEAT (2a leva, 'seguir') · A tabela Por base ganhou o resto do que o consolidado nao respondia: (1) o delta no mesmo recorte passou a valer tambem em Cargas, Receita CTE e Margem -- mora embaixo do proprio numero em vez de virar uma coluna por metrica, senao a tabela dobrava de largura; (2) ORDENAVEL pelo cabecalho (colunas declaradas como dado, entao coluna nova nao precisa ser escrita em 3 lugares); (3) coluna ALERTAS por base -- a lista de alerta do topo ja carrega o registro, entao da pra dizer de qual base o incendio veio; (4) Top Destinos passou a mostrar a origem por base ('Imperatriz/Belem 12 · Maracanau 3'), porque destino sozinho nao diz de onde saiu a carga.",
      "FIX · Delta de MARGEM com margem anterior negativa invertia o sinal (sair de -100 pra -50 e melhora e aparecia como queda). Denominador passou a ser o modulo do valor anterior.",
      "FIX (achado ao abrir alerta por base) · No consolidado o motor de alerta era o da base ATIVA (padrao), entao linha da AVB caia em alerta de descarga/saldo, que aquela operacao nem tem. Passou a resolver o perfil por linha (getPerfil(r._baseId)) quando a linha traz base; fora do consolidado nada muda.",
  ...
  },
  {
    ...
    itens: [
      ...
      "UX (Yves: 'nao podemos criar campos novos para cada solicitacao') · O acesso NAO virou item de sidebar: mora dentro do proprio modal do WhatsApp, como 3a opcao da lista que ja escolhe o modelo (Faturamento / Contratacao / Preencher faturamento) -- mesma DT, mesmos campos, sentido inverso. E colar o bloco direto no campo de BUSCA daquele modal ja abre o preenchimento, porque a DT vem do texto e exigir busca antes seria trabalho a toa. Opcao escondida em base ancorada por codigo (AVB).",
      "REGRA · DATA MANIFESTO nao vem do texto: e preenchida pela tela com a data do LANCAMENTO, editavel pra quando o faturamento foi feito em outro dia (default = a data que o registro ja tinha, senao hoje).",
      "ARQUITETURA · A gravacao escreve PRIMEIRO na planilha e so depois no Supabase. Motivo medido: a sync e planilha->Supabase e sobrescreve por DT a cada 15 min -- celula vazia vira '' no upsert, entao gravar so no banco seria apagado na rodada seguinte, sem aviso. Se a planilha nao aceitar, o app NAO grava e diz o porque. Caminho: browser -> /api/sheets-write (proxy na Vercel, esconde o token e evita CORS) -> doPost do Apps Script -> celula. O doPost so aceita os 6 campos de faturamento, usa o mesmo mapearColuna() da leitura e trava com LockService.",
      "PENDENTE DO YVES · (1) colar o SyncSupabase.gs novo, definir WEBAPP_TOKEN e publicar como App da Web (Executar como: eu · Acesso: qualquer pessoa); (2) na Vercel, SHEETS_WEBAPP_URL_IMPERATRIZ_BELEM = URL /exec e SHEETS_WEBAPP_TOKEN = mesmo token. Sem isso o modal recusa a gravacao com a mensagem de nao configurado (de proposito -- melhor recusar que perder o dado na sync).",
      "ACHADO/FIX (o proprio log da sync entregou) · 15 cabecalhos da planilha NAO eram lidos e os campos correspondentes estavam CONGELADOS no banco -- preenchidos um dia, e desde entao a planilha mudava e o banco nao acompanhava: dias 432 linhas, rdo 294, hr_ro 351, diaria 357, pag_desc 235, pag_stretch 114, total 95, comprovei 325, cad_fortes 269, data_lib 146, diaria_rec 66. Era so ortografia do cabecalho ('QUANT.DIAS'/'Qtd DIAS' x 'dias', 'DIARIAS RECEBIDO' x nada, 'PAG. DESCARGA' x nada). Aliases adicionados no mapearColuna + os 5 campos de dinheiro novos entraram em CAMPOS_FINANCEIROS (senao celula-numero viraria formato americano e o parser BR inflaria o valor, o mesmo bug de 2026-07-17).",
      "FIX (efeito colateral do mapeamento novo, pego conferindo o dado gravado) · tratarValor() centraliza como celula vira texto: (1) HORA PURA vira HH:mm -- o Sheets guarda hora sem data como 30/12/1899, e formatar em dd/MM/yyyy jogou '30/12/1899' em ~660 linhas de HR RO assim que a coluna passou a ser lida; (2) erro de formula (#VALUE!, #REF!, #N/A...) entra VAZIO em vez de virar 'valor' -- havia #VALUE! em diaria e total. Os dois caminhos (rodada completa e busca de um DT) usam a mesma funcao, e a proxima rodada limpa sozinha o que ja entrou errado.",
      "VALIDADO · Depois de colar e publicar: dias 432->1099 linhas, hr_ro 351->1011, comprovei 325->1058, diaria 357->1083, diaria_rec 66->549, pag_desc 235->497, pag_stretch 114->340, total 95->942. ZERO valores em formato americano nos 5 campos de dinheiro novos (a guarda do CAMPOS_FINANCEIROS pegou).",
      "REGRA · Ficaram DE FORA de proposito os cabecalhos que o app tambem edita (FORMS e DIARIAS PAGAS): mapear faria a planilha sobrescrever a cada 15 min o que foi digitado no app, que e exatamente o efeito colateral que o write-back existe pra evitar. Decisao de quem manda nesses dois campos e de operacao, nao de codigo.",
      "FEAT (Yves: 'quando o tempo entre por a DT na planilha e vir preencher aqui e curto, o app nao acha a DT') · Botao BUSCAR ESTA DT NA PLANILHA AGORA, que aparece dentro do proprio aviso de 'DT nao existe nesta base'. Puxa SO aquela linha (acao sincronizar_dt no mesmo Web App ja publicado -> localizarLinhaPorDT -> upsert no Supabase) e injeta o registro no DADOS local, entao a conferencia aparece na hora. Nao e um 'force sync' geral de proposito: reprocessar as ~850 linhas levaria dezenas de segundos e estouraria o tempo da funcao na Vercel, enquanto uma linha volta em segundos.",
      "REFACTOR · localizarLinhaPorDT() saiu de dentro do write-back: achar a linha do DT (deteccao de cabecalho + varredura das abas) agora serve as duas direcoes -- escrever campos e puxar a linha inteira. A puxada aplica data/moeda e a limpeza de origem/tipo_carga, mas nao as regras de fila sem-DT e celulose: a rodada completa passa por cima dessa linha em ate 15 min de qualquer jeito.",
      "FIX (colaborador do Yves, usando o bloco de CONTRATACAO) · Modal 'estatico', sem rolagem, escondendo parte da conferencia -- justamente no bloco maior (14 campos). Causa: a altura maxima era 100vh, que no celular INCLUI a barra do navegador; o modal ficava mais alto que a area visivel, o rodape saia da tela e o corpo nao rolava porque, pra ele, o conteudo cabia. Passou a usar dvh (area realmente visivel) com vh de reserva, o overlay ganhou rolagem propria com margin:auto no modal (com alignItems:center puro, conteudo alto corta o topo e fica inalcancavel) e os corpos ganharam flex '1 1 auto' + minHeight 0 + overscroll contido. VALIDADO fora do app, replicando os estilos: com o modal preso em 560/420/320px o corpo rola ate o fim, header+corpo+rodape somam a altura do modal e o rodape nunca sai dele.",
      "UX (Yves: 'as placas em contratacao estao vindo juntas e temos tres campos no Sheets') · O card de Contratacao escrevia 'PLACAS: KEW9943 / KQW5I51' numa linha so, e quem recebia tinha que separar de novo na hora de lancar. Agora sai UMA LINHA POR PLACA (PLACA 01/02/03), casando 1:1 com as tres colunas da planilha e com placa/placa2/placa3 do banco. O bloco colado ganhou os tres rotulos separados (aceita 'carreta' como placa 02) e CONTINUA aceitando o formato antigo junto, que quebra na barra -- ninguem precisa refazer texto salvo. Placa 04 (existe no cadastro do motorista, nao na planilha) vira aviso explicito em vez de sumir calada.",
      "UI (Yves: 'pq a tela fica colada na parte de baixo?') · Modal deixou de ser bottom-sheet e passou a FLUTUAR CENTRALIZADO -- no desktop o conteudo caia no pe da tela com o topo vazio. Mudanca no css.overlay/css.modal (useCss), entao vale pros 8 modais que usam esse estilo, mais o drill do dashboard, que era o unico ainda usando a classe .co-modal-overlay sem --center. Cantos arredondados nos 4 lados, sombra em vez de borda colada, animacao slideUp no lugar do mslide (que subia do rodape) e altura = 100vh - 40px do padding do overlay: os overrides antigos de 94/96vh estouravam a tela agora que ha respiro em volta.",
      "FEAT (pedido Yves) · FORMA DE PAGAMENTO no bloco de contratacao (PGTO: cheque | conta | ambos). Levantamento pedido antes de criar: o banco NAO tinha onde guardar isso -- pag_desc/pag_stretch sao VALORES de descarga/stretch, motoristas.pix_* e dado bancario de quem recebe, e os 3 botoes do card viviam so no estado da tela (wppPgto) e morriam ao fechar o modal. Migration 062 cria forma_pgto (controle_operacional + maracanau) com o mesmo vocabulario dos botoes. O parser aceita o que o card imprime ('PGTO: ✅ CHEQUE + CONTA' vira 'ambos') e recusa o que nao reconhece, dizendo quais valores valem.",
      "REGRA · forma_pgto e o primeiro campo SO DO APP: a planilha nao tem essa coluna, e por isso mesmo e seguro guardar so no Supabase -- o sync monta o upsert apenas com as colunas que mapearColuna() conhece, entao o que nao existe la nunca e sobrescrito na rodada de 15 min. A gravacao separa os campos em 'vai pra planilha' e 'so app' (CAMPOS_SO_APP).",
      "FEAT (pedido Yves, 13/08) · BLOCO DE CONTRATACAO colado, irmao do de faturamento: DT / ID / NOME / CPF / TELEFONE / PLACAS / DESTINO / CARREGAR / AG DESCARGA / VLR EMPRESA / VLR MOT / ADT -- espelho do card de Contratacao que o app ja gera, mais o ID. Placas vem juntas ('KEW9943 / KQW5I51') e viram placa/placa2/placa3; datas normalizam pra dd/MM/yyyy; dinheiro perde o 'R$' e entra COMO ESTA no texto (o app guarda moeda como texto e o formato varia por base -- reformatar aqui seria inventar numero). DT que nao existe continua sendo recusada: carga nasce na planilha ou no Nova DT, colar nao e porta de entrada.",
      "UX · Em vez de uma 4a opcao na lista do WhatsApp, a entrada continua UMA ('Preencher pelo bloco') e o app RECONHECE qual bloco e pelo proprio texto, contando os rotulos de cada um; dois chips no topo do modal mostram o reconhecido e deixam trocar na mao. Colar um bloco no modo errado nao some com o dado: cada linha vira aviso 'e campo de Contratacao -- ignorado neste bloco'.",
      "FIX · O card que o app gera poe dois campos na MESMA linha ('DT: 123    DESTINO: X'). Colar ele de volta gravava dt='1348169DESTINOBRASILIA-DF' e perdia o destino. fatiarLinha() corta a linha em cada rotulo conhecido seguido de ':', entao o texto que sai do app volta inteiro.",
      "DB/SYNC · mapearColuna passou a mapear PLACA 02/03 (placa2/placa3): as colunas existiam na planilha e no banco mas nunca eram lidas, e sem elas o write-back da contratacao nao acharia onde escrever a carreta. Whitelist de escrita (Apps Script + /api/sheets-write) cobre agora os campos dos DOIS blocos.",
      "REGRA (pedido Yves, mesmo dia) · ID SAIU do bloco colado e CLIENTE entrou: o ID passou a ser preenchido pelo contratante, entao cobrar aqui criaria pendencia falsa no KPI. Ordem nova: DT / CTE / MDF / MAT / NF / CLIENTE. Quem colar o formato antigo nao perde nada em silencio -- o rotulo ID continua reconhecido so pra avisar que agora vem do contratante. Cliente e texto (aceita espaco e acento) e fica fora da leitura 'sem rotulos', que so vale pros numericos.",
      "FEAT · KPI 'SEM FATURAMENTO' no dashboard: conta as DTs do periodo a que falta QUALQUER um dos 6 (CTE, MDF, MAT, NF, Cliente, Data Manifesto) e abre o drill listando cada DT com os badges do que falta. Numeros de hoje na Imperatriz/Belem: 9 em 08/2026, 17 em 07/2026, 13 em 06/2026. Entra no catalogo de KPIs (dashboardConfig), entao da pra desligar por usuario.",
      "PRONTO PRA IA · parseFaturamento() devolve o contrato { campos, achados, avisos }; plugar a extracao do yf-ai-gateway e trocar quem produz `campos` -- a tela de conferencia e a gravacao nao mudam.",
  ...
  },
  {
    ...
    itens: [
      ...
      "UX (pedido Yves) · KPI 'EM REVISAO' com o VALOR em aberto entra na faixa de indicadores do Resultado, ao lado de Resultado/Creditos, e e clicavel: filtra a lista so com os marcados e rola ate ela (clicar de novo volta a lista inteira). Enquanto o filtro esta ligado, o card muda pra 'Em revisao - filtrando', o cartao da lista ganha borda em ouro e a OBSERVACAO de cada linha sai do tooltip e aparece na tela -- e o que a pessoa precisa ler pra decidir. Card so aparece quando ha algo em revisao.",
      "REGRA · em_revisao NAO mexe em numero nenhum: a despesa continua entrando (ou nao) no resultado exatamente como estava -- e so a marca de 'isto ainda vai ser conferido', mesma filosofia das flags da Conferencia de Faturamento. Na lista aparece o badge EM REVISAO (com a observacao no tooltip) e um chip 'Em revisao (N)' ao lado do 'Todos os meses' filtra so as pendentes; o chip so aparece quando ha alguma.",
      "FIX (Yves: planilha com formatacao errada entrou e gravou CTe pela metade) · O parser le por NOME de coluna, entao um export com outro layout nao dava erro: cada coluna ausente virava vazio/zero e a linha entrava mutilada. Foi o caso de um relatorio reduzido do AVB (15 colunas, sem Data Emissao, Total do Frete, Trecho, NFS, Placa, Peso NF, Valor NF, Manifesto e Contrato) -- o CTRC 35012 ficou sem data, sem placa e com Total do Frete zerado. Agora a importacao EXIGE as colunas essenciais (Empresa, CTRC, CNPJ Remetente, Data Emissao, Frete Peso, Total do Frete, Valor Contrato Frete, Saldo) e recusa o arquivo dizendo exatamente quais faltam.",
      "FEAT · Reimportar o arquivo CERTO agora CORRIGE o que ja esta gravado (migration 059 + atualizarFreteLote): antes o import so inseria o que era novo, entao a linha errada ficava assim pra sempre -- a chave (cnpj+categoria+ctrc+periodo) ja existia e o arquivo bom passava batido. A atualizacao toca SO nos campos do documento (os que vem do TMS) + flags automaticas; decisao de revisao, categoria definida a mao, vinculo de ciclo de vida, competencia e contrato apontado a mao ficam intactos. Contrato vinculado a mao tambem nao volta pra fila. O toast diz quantas linhas foram atualizadas e quais CTRCs.",
      "UX (pedido Yves) · Rodape do modal do CTe virou UMA barra encostada a direita, do menos pro mais importante: Fechar / Editar / Excluir / Estornar, um traco divisor, e entao Vincular contrato / Descarga-Local / Sinalizar / Marcar revisado (primario por ultimo). O desenho anterior ancorava um grupo a esquerda e outro a direita, o que jogava as duas linhas em cantos opostos quando o rodape quebrava. Ganhou tambem borda de topo e alinhamento vertical central; o traco some no modo 'revisando', quando nao sobra decisao a direita.",
      "REGRA (Yves, CTRC 34973) · CTe que SUBSTITUI documento de mes ja FECHADO nao pode entrar como lucro: aquele faturamento ja foi para o fechamento e para a base de comissao do mes passado, entao o substituto com contrato zerado conta o mesmo dinheiro duas vezes. O certo e lancar o contrato no MESMO valor do CTe SEM ICMS, pro CTe apenas dar baixa e fechar com saldo zero. O valor sem ICMS sai dos proprios numeros: Saldo = Total - (contrato + ICMS), entao (saldo + contrato) e o CTe menos o ICMS -- no 34973, R$ 568,18 - R$ 68,18 = R$ 500,00.",
      "FEAT · substituicaoDeMesFechado() em freteConferencia.js + bloco proprio no modal: diz qual CTRC foi substituido e de que mes, quanto lancar de contrato de baixa (com a conta do ICMS na tela) e tem atalho que abre o 'Sinalizar para correcao' com a justificativa pronta. Fica verde quando a baixa ja esta lancada. O aviso generico de 'contrato zerado' sai de cena nesse caso -- ele mandaria lancar contrato de terceiro, que e o conselho errado aqui. Par que nao esta na base conta como mes fechado (CTe de mes nao importado ja passou). Hoje ha 1 caso: o proprio 34973.",
      "FEAT (Yves: 'pq nao podemos clicar e vincular?') · O item 'CTE SEM CONTRATO, MAS O CONTRATO EXISTE' da fila de Contratos ganhou o botao 'Vincular ao CTe X' -- antes so dava pra vincular pelo modal do CTe na Conferencia, ou seja, achar de novo o mesmo CTe em outra tela pra dar o mesmo clique. O vinculo grava contrato_ref no CTe do grupo que esta zerado (cte_alvo). Depois de vinculado o caso muda de estado: vira 'Vinculado a mao -- falta lancar no TMS' (verde), mostra o saldo real e oferece desfazer, porque o dinheiro so entra na margem de verdade quando o TMS for corrigido e o relatorio reimportado.",
      "FIX (Yves: 'Algo deu errado nesta tela' ao abrir a Conferencia) · Dois usos antes da declaracao no mesmo arquivo: relLinhas lia linhasFiltradas e clientesPresentes lia clientesDaFilial. `const` nao sobe, entao a tela morria na montagem com 'Cannot access before initialization' e caia no ErrorBoundary -- e o build passa, porque e erro de execucao. Ordem corrigida: clientesDaFilial -> clientesPresentes -> linhasFiltradas -> bloco do relatorio. Nada de logica mudou.",
      "FEAT (pedido Yves) · VINCULAR CONTRATO no modal do CTe (migration 058): quando o TMS emite o CTe sem amarrar no contrato, o CTe aparece com contrato zerado e margem 100% mesmo existindo contrato lancado. Agora da pra apontar qual e -- o modal lista os contratos JA IMPORTADOS que combinam (mesmo CTRC apontado no relatorio de contratos, mesma placa, mesmo valor total, mesma data), com o rotulo 'aponta este CTe' no caso obvio, e aceita digitar o numero. Caso real: CTRC 34939 (AVB) esta com contrato 0 e o contrato 26844 (ARISUL, R$ 10.284,52) aponta pra ele -- mesma placa JDG0F31, mesmo trecho, mesma data.",
      "REGRA · O numero do TMS NAO e sobrescrito: o vinculo grava contrato_ref (coluna nova) e quem le usa numeroContratoDoCte() = numero_contrato do TMS, ou o vinculado quando aquele veio vazio/0. Assim, se o relatorio vier corrigido depois, da pra ver se os dois batem. Vincular tira a flag SEM CONTRATO; desfazer devolve a linha a regra automatica. O bloco mostra a margem REAL (frete - contrato) antes de decidir.",
      "UX · Rodape do modal do CTe reorganizado em dois grupos: a esquerda o que mexe no REGISTRO (Fechar, Editar, Excluir, Estornar) e a direita a DECISAO da conferencia (Vincular contrato, E Descarga/E Local, Sinalizar, Marcar revisado). Antes vinha tudo numa fileira so e 'Excluir CTe' ficava colado em 'Marcar revisado'.",
      "FEAT (pedido Yves) · MODAL DE RELATORIO REUTILIZAVEL (src/components/ModalRelatorio.jsx): recebe as linhas que a tela JA filtrou + a definicao das colunas e resolve o resto -- ligar/desligar coluna, ordenar clicando no cabecalho, agrupar com subtotais, total geral, export XLSX/CSV e imprimir em A4 paisagem (janela propria, sem o CSS do app). Plugado na Conferencia de Faturamento (respeita filial/cliente/mes, so CTes ativos) e em Contratos (contratos nossos do mes, agrupavel por cliente/PF-PJ/trecho/situacao). O ReportBuilder do modulo de relatorios continua existindo pra montar relatorio campo a campo do OPERACIONAL -- ele e amarrado aquele catalogo, por isso nao deu pra reusar aqui. Fase seguinte: aba de IA no mesmo modal (resumo automatico + pergunta livre), que precisa de um perfil de analise no yf-ai-gateway.",
      "FEAT (pedido Yves) · SELETOR DE FILIAL NO TOPBAR: o menu de base passou a listar 'Imperatriz / Belem' (as duas juntas, que segue sendo o padrao da gestao) + 'Imperatriz' + 'Belem' como sub-itens. A escolha vira estado global (filialAtiva, persistido em localStorage) e desce pelo ctx, no lugar do estado local que cada tela tinha. As sub-opcoes so aparecem em base com features.filialNasDespesas -- nao ha 'imperatriz_belem' hardcoded na UI.",
      "REFACTOR · Resultado e Painel Financeiro perderam o segmentado proprio de filial (era o mesmo recorte repetido na mesma faixa do topbar) e passaram a ler ctx.filialAtiva, mostrando so um chip com a filial ativa. Nada mudou no calculo: continua origemBate() para a receita e aba_origem para a despesa.",
      "DB · Migration 057 (APLICADA 2026-08-11): coluna embarcadoras.filial ('IMP'|'BELEM'|NULL). A Confererencia nao recorta por origem de viagem e sim por EMBARCADORA, entao o vinculo cliente->filial precisava ser explicito no cadastro -- inferir pelo nome quebraria no primeiro cliente novo. Seed: SUZANO FAB IMPERATRIZ=IMP, SUZANO FAB BELEM=BELEM. Na tela, a filial recorta as linhas e a lista de clientes, e o seletor de embarcadora CONTINUA (filial escolhe a filial; o seletor escolhe um cliente dentro dela), limpando a selecao se o cliente nao for da filial ativa.",
      "FIX (achado do Yves no CTRC 34927) · UM contrato pode virar DOIS CTes: viagem com duas entregas (duas NFs, cidades diferentes) sai em CTes separados com o MESMO No Contrato Frete e o mesmo manifesto, e o TMS lanca o contrato inteiro em um deles, zerando o outro. Olhando CTe a CTe o segundo virava falso 'SEM CONTRATO' e a margem saia errada nos dois. Caso real: contrato 26833 (Rodorrica), CTes 34926 + 34927 -- o app mostrava saldo de R$ 3.109,87 e margem 52,5% em cada, quando o grupo deixa R$ 300,00 (5,1%, que e a regra da frota Rodorrica).",
      "REGRA · freteConferencia.js ganhou gruposPorContrato/irmaosDoContrato/resumoGrupoContrato/contratoEstaNoIrmao (chave: empresa_cod + numero_contrato, ignorando vazio e '0'). flag_sem_contrato deixou de disparar quando o contrato esta num irmao do mesmo contrato. No modal do CTe entra o bloco 'Contrato N x M CTes' com frete/contrato/saldo/margem do GRUPO + badge. Varredura da base: 45 grupos multi-CTe em 2026, 101 CTes, 54 com contrato zerado -- todos estavam na fila como falso positivo e foram corrigidos por UPDATE. A fila de 07-08/2026 caiu de 15 para 9 itens, e os 9 que sobraram sao contrato realmente nao lancado.",
      "REGRA · O cruzamento de contratos passou a casar 1:N: alem do 'CTe Ctrc' que o relatorio aponta, junta todos os CTes com o mesmo No Contrato Frete. 'CTe sem contrato' so acusa se NENHUM CTe do grupo recebeu valor, e a tela mostra 'cobre N CTes' com frete e saldo do conjunto.",
      "FEAT Fase 3 (pedido Yves, relatorio MAR) · O relatorio de contratos traz TUDO que a empresa de emissao rodou, nao so a nossa operacao: no MAR de 01-11/08/2026 sao 81 contratos e so 11 casam com CTe da base (trecho BEMSZP, Suzano Belem); os outros 69 sao trechos MAB* (Maraba) e ANN*, R$ 623.429 contratados. Sem separar, o 'encargo faltando' daria R$ 16.074,64 em vez dos R$ 448,00 reais.",
      "REGRA (decisao Yves) · NAO travar em 'so Suzano 16404287069864' -- com outra embarcadora no futuro teria que mexer no codigo. Migration 056: tabela contratos_regras_trecho ((empresa_emissao, trecho) -> ignorar) + RPCs listar/definir. Quem casa com CTe e nosso por definicao (o cliente vem do CTe, seja quem for); a regra so decide o que NAO casou. Trecho sem regra cai em 'trecho_nao_decidido' e a tela PERGUNTA -- e assim que embarcadora nova aparece em vez de sumir em silencio. Seed da decisao de hoje: BEMSZP e nossa, os 12 trechos MAB*/ANN* ficam de fora.",
      "UX · Bloco 'trechos sem decisao' no topo da tela de Contratos, agrupado por trecho (26 contratos de MABCUC viram UMA pergunta, nao 26), com 'E nossa operacao' / 'Nao e nossa'; a resposta fica gravada e vale pros proximos meses. Rodape com os trechos ignorados do mes e um clique pra reverter, pra decisao nao virar caixa-preta. Ignorado sai de TODOS os numeros (senao o encargo faltando contaria dinheiro de terceiro), aparecendo so como contagem no KPI.",
      "MEDIDO no MAR com as regras aplicadas · 12 contratos nossos (11 com CTe + 1 aguardando), 69 fora, 7 PF sem custos externos = R$ 448,00 faltando, 1 contrato zerado (CTRC 87955, a diaria emitida ja marcada com competencia 06/2026) e 1 sem CTe na base (CTRC 87999, 11/08, Transporte Rebeca: e nosso, o relatorio de CTes de Belem so vai ate 09/08). INSS e SEST/SENAT lancados em todos.",
      "FEAT Fase 2 · Tela CONTRATOS: novo segmento na faixa do Resultado (Operacional / Conferencia de Faturamento / Contratos), em src/views/ContratosFrete.jsx. Importa o .xlsx de contratos com preview (linhas, empresa, periodo, novos x atualizados -- reimportar ATUALIZA, entao depois de lancar o que faltava o numero se corrige sozinho), 4 KPIs (contratos, valor contratado, encargo lancado, encargo faltando) e fila de pendencias filtravel por tipo, ordenada por quanto falta em dinheiro. Cada item mostra o calculo com o numero na mao (esperado x lancado por rubrica) pra pessoa lancar sem abrir o TMS pra descobrir. App.jsx passou a injetar setContratosToken junto com os outros tokens de sessao. Build ok; carregamento sem erro de console verificado no dev server, mas a tela em si NAO foi validada logada (precisa de credencial).",
      "FEAT Fase 1 (pedido Yves) · CONTRATOS DE FRETE: o relatorio de CTes diz o que se COBRA, mas nao diz quem levou nem quanto custou de encargo. O relatorio de contratos do TMS traz CPF/CNPJ do agregado, Valor INSS, SEST/SENAT, Custos Externos e a coluna 'CTe Ctrc', que casa 1:1 com frete_conferencia.ctrc. Fase 1 entregue: migration 055 (tabela frete_contratos + RPCs listar/inserir/excluir, RLS sem policy igual frete_conferencia) e src/freteContratos.js (parser do .xlsx, diff/upsert, cruzamento e resumo). Fase 2 (tela de importacao + fila de pendencias) fica pra proxima sessao.",
      "MEDIDO no relatorio 01-06/08/2026 (MAT, 54 linhas) · 35 contratos de pessoa fisica x 19 de PJ; nenhum PJ tem encargo (correto). As aliquotas batem exatas nos 32 PF com valor lancado: INSS 2,2% e SEST/SENAT 0,5% do contrato (11% e 2,5% sobre a base de 20% do frete). O furo esta em CUSTOS EXTERNOS (4%, cota patronal): lancado em so 13 de 32 -- 19 contratos sem, R$ 114.687,18 contratados, R$ 4.587,49 de encargo que nao aparece em lugar nenhum e faz a margem do frete parecer melhor do que e. Onde tem, bate no centavo (R$ 2.490,02 x R$ 2.490,19 de 4% teorico).",
      "REGRA · Casamento e SEMPRE por CTRC + empresa de emissao, nunca por cliente: MAT e a MATRIZ e mistura Imperatriz (30 CTes), Acailandia (20) e Maranhao Ind. de Couros (3) no mesmo arquivo. Teste do parser contra o arquivo real: 54 linhas lidas, 53 com CTe, 52 acharam o CTe em 08/2026 (o 53o, CTRC 34869, e de 07/2026 -- casa igual, so esta fora do mes) e o contrato bate com o app em 50. Tolerancia de R$ 0,50 na conferencia das aliquotas: o TMS arredonda por fora e centavo nao e lancamento faltando.",
      "ACHADO · CTRC 34939 (AVB, ARISUL PJ): o CTe veio com contrato ZERADO e margem 100% -- foi um dos que a flag SEM CONTRATO pegou. O contrato existe no outro relatorio: 26844, R$ 10.284,52. O cruzamento nao so aponta como RESOLVE, entregando o valor que falta. Ja o CTRC 34942 (a diaria agregada de R$ 38.800) tem contrato lancado COM VALOR ZERO -- consistente com o app, nao e erro de importacao.",
      ...
      "REGRA (adendo Yves) · O codigo da diaria e POR CLIENTE: D01 = Suzano Imperatriz, D05 = Suzano Belem (diaria_cod no cadastro da embarcadora, ja correto na base). Como candidatosVinculo filtra por cliente, uma diaria emitida de Belem nunca casa com um D01 de Imperatriz -- nao precisou mudar regra, so documentar. CTRC 87955 (diarias a receber de Belem) ja tinha entrado sozinho como diaria_emitida pela regua (MAR, R$ 800, peso 0, sem NF) e recebeu competencia 06/2026 (informado pelo Yves): junho de Belem pagou R$ 2.000 e recuperou R$ 800 = 40%.",
      "REGRA (adendo Yves) · O D01/D05 e emitido ANTES ou DEPOIS do espelho de diaria, entao o que foi PAGO num mes pode se referir a dois meses de espelho diferentes. Migration 054 (APLICADA 2026-08-10): competencia_ref passou a valer tambem em categoria='diaria' (mes do espelho), nao so na emitida. O card frete x diaria le as DUAS pontas pela competencia (mesCompetencia, ex-mesDaEmitida) -- antes so a receita era corrigida e a comparacao errava do lado do custo. Modal do D01/D05 ganhou o mesmo bloco, com atalho de mes anterior E mes seguinte, e badge 'ESPELHO DE MM/AAAA'.",
      "ACHADO · Belem segue sem NENHUM D05 em 07 e 08/2026 (ultimo foi 06/2026, 2 CTes de R$ 2.000) -- a pendencia levantada na sessao 52 continua aberta. Efeito pratico: as diarias emitidas de Belem (400 em 07, 800 em 08) nao tem custo par na base, entao a margem de Belem aparece melhor do que e. O modal agora avisa quando uma diaria emitida nao tem NENHUMA diaria paga do cliente nos meses carregados, em vez de deixar a pessoa procurando um par que nao existe.",
    ],
  },
  {
