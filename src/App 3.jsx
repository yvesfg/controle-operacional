...
export default function App() {
  ...
  return (
    <div style={css.app}>
      ...
      {/* TAB BAR */}
      <div style={{...css.tabBar,justifyContent:"space-between"}}>
        ...
        {/* Botão WhatsApp fixo na barra */}
        <div style={{position:"relative",flexShrink:0}}>
          <button
            onClick={()=>setWppTipoOpen(v=>!v)}
            style={{flex:"0 0 auto",padding:"10px 14px",fontSize:10,fontWeight:700,letterSpacing:.5,textTransform:"uppercase",color:"#25D366",border:"none",background:`rgba(37,211,102,.12)`,cursor:"pointer",borderRadius:10,whiteSpace:"nowrap",transition:"all .2s",boxShadow:`0 0 0 1.5px rgba(37,211,102,.3) inset`,display:"flex",alignItems:"center",gap:5,fontFamily:"inherit"}}
          >
            <span style={{fontSize:14}}>📲</span> WHATSAPP ▾
          </button>
          {wppTipoOpen && (
            <div style={{position:"absolute",right:0,top:"110%",background:t.card,border:`1px solid ${t.borda}`,borderRadius:12,overflow:"hidden",zIndex:200,minWidth:200,boxShadow:`0 8px 28px ${t.shadow}`,animation:"slideUp .2s"}}>
              {[
                {k:"faturamento",ico:"🧾",l:"Faturamento",sub:"CTE · MDF · MAT · DT · NF · ID"},
                {k:"contratacao",ico:"📝",l:"Contratação",sub:"Modelo completo de pagamento"},
                {k:"descarga",ico:"📦",l:"Descarga / Stretch",sub:"Solicitar pagamento descarga"},
                {k:"diarias",ico:"🛏️",l:"Diárias",sub:"Solicitar pagamento diária"},
              ].map((op,i,arr)=>(
                <button key={op.k} onClick={()=>{setWppTipoOpen(false);}} style={{width:"100%",background:"transparent",border:"none",borderBottom:i<arr.length-1?`1px solid ${t.borda}`:"none",padding:"10px 14px",color:t.txt,fontSize:12,fontWeight:600,cursor:"pointer",textAlign:"left",fontFamily:"inherit",display:"flex",alignItems:"flex-start",gap:10,transition:"background .15s"}}
                  onMouseOver={e=>e.currentTarget.style.background=t.card2}
                  onMouseOut={e=>e.currentTarget.style.background="transparent"}
                >
                  <span style={{fontSize:18,flexShrink:0}}>{op.ico}</span>
                  <div>
                    <div style={{color:t.txt,fontWeight:700}}>{op.l}</div>
                    <div style={{fontSize:9,color:t.txt2,marginTop:1}}>{op.sub}</div>
                  </div>
                </button>
              ))}
              <div style={{padding:"8px 14px",borderTop:`1px solid ${t.borda}`,fontSize:9,color:t.txt2}}>
                💡 Busque o registro primeiro (aba Busca) para usar estes modelos com dados preenchidos
              </div>
            </div>
          )}
        </div>
      ...
      <div style={{padding:"16px 16px 100px",maxWidth:1100,margin:"0 auto",animation:"fadeIn .2s"}}>
            ...
            {dSubTab === "resumo" && (
              <div>
                ...
                <div style={{display:"flex",gap:5,marginBottom:8,flexWrap:"wrap",alignItems:"center"}}>
                  {[{k:"todos",l:"Todos"},{k:"diaria",l:"🛏️ Com Diária"},{k:"atraso",l:"⚠️ Perdeu agenda"},{k:"sem_diaria",l:"✅ Sem diária"},{k:"pendente",l:"⏳ Aguardando"}].map(f => (
        ...
        {activeTab === "operacional" && (()=>{
          ...
          const subBtns = [
            {k:"sgs",ico:"📞",l:"SGS · Chamados",sub:"Chamados SGS"},
            {k:"diarias_id",ico:"🛏️",l:"ID Diárias",sub:"IDs vinculados"},
            {k:"ocorrencias",ico:"⚠️",l:"Ocorrências",sub:"Histórico geral"},
            {k:"apontamentos",ico:"📑",l:"Apontamentos",sub:"Descargas/Stretch"},
          ];
          ...
          return (
            <div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:8,marginBottom:16}}>
                {subBtns.map(sb=>(
                  <button key={sb.k} onClick={()=>setOperSubTab(sb.k)} style={{border:`1.5px solid ${operSubTab===sb.k?t.azul:t.borda}`,borderRadius:11,padding:"12px 10px",cursor:"pointer",background:operSubTab===sb.k?`rgba(22,119,255,.08)`:t.card,display:"flex",flexDirection:"column",alignItems:"center",gap:4,transition:"all .2s",fontFamily:"inherit"}}>
                    <span style={{fontSize:22}}>{sb.ico}</span>
              ...
              {operSubTab==="sgs" && (
                <div>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                    <div style={{...css.secTitle,margin:0}}>📞 Chamados SGS <span style={{flex:1,height:1,background:t.borda}} /></div>
              ...
              {operSubTab==="diarias_id" && (
                <div>
                  <div style={{...css.secTitle,marginBottom:12}}>🛏️ ID Diárias <span style={{flex:1,height:1,background:t.borda}} /></div>
              ...
              {operSubTab==="ocorrencias" && (
                <div>
                  <div style={{...css.secTitle,marginBottom:12}}>⚠️ Ocorrências Gerais <span style={{flex:1,height:1,background:t.borda}} /></div>
              ...
              {operSubTab==="apontamentos" && (
                <div>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                    <div style={{...css.secTitle,margin:0}}>📑 Apontamentos <span style={{fontSize:8,color:t.txt2,fontWeight:400,marginLeft:4}}>Descargas/Stretch</span> <span style={{flex:1,height:1,background:t.borda}} /></div>
          ...
          <div style={css.overlay} onClick={e=>e.target===e.currentTarget&&setWppFatModal(null)}>
            <div style={{...css.modal,maxHeight:"90vh"}}>
              ...
              <div style={{flex:1,overflowY:"auto",padding:14,display:"flex",flexDirection:"column",gap:10}}>
                ...
                {(!reg.cte||!reg.mdf||!reg.mat||!reg.nf) && <div style={{background:`rgba(240,185,11,.07)`,border:`1px solid rgba(240,185,11,.25)`,borderRadius:8,padding:"8px 10px",fontSize:10,color:t.ouro}}>⚠️ Alguns campos estão vazios. Edite o registro para preenchê-los.</div>}
              </div>
              <div style={{padding:"10px 14px 18px",borderTop:`1px solid ${t.borda}`,display:"flex",gap:8}}>
