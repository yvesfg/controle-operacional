...
# ── Create src/hooks/useAuthHandlers.js ─────────────────────────────────────
hook_js = r"""import { useCallback } from "react";
import { loadJSON, saveJSON, hashSenha, verificarSenha, supaFetch } from "../utils.js";
import { BASES, TABLE_USUARIOS, TABLE_CONFIG, ENV_SUPA_URL, ENV_SUPA_KEY, PERMS_PADRAO } from "../constants.js";
...
export function useAuthHandlers({
  ...
  setPrimeiroLogin, primLoginSenha, primLoginSenha2,
  ...
  const handleLogout = () => {
    registrarLog("LOGOUT", `${/*usuarioLogado passed via closure*/""||"usuário"} saiu do sistema · ${new Date().toLocaleString("pt-BR")}`).catch(()=>{});
    // Invalida token server-side (M2/M3)
    const conn = getConexao();
    if (conn && setSessionToken._current) {
      supaFetch(conn.url, conn.key, "POST", "rpc/logout_usuario", {p_token: setSessionToken._current}).catch(()=>{});
    }
...
print("NOTE: handleLogout needs sessionToken access - will fix App.jsx to pass it and usuarioLogado")
